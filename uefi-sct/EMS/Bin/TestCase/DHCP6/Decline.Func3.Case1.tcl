...
#
CaseGuid        46BC4B56-9F97-4726-90F9-417285B6740A
CaseName        Decline.Func3.Case1
CaseCategory    DHCP6
...
# EndScope
#
EndScope _DHCP6_DECLINE_FUNC3_
#
# End Log 
#
EndLog
...
# BeginScope
#
BeginScope  _DHCP6_DECLINE_FUNC3_
...
SetVar R_ConfigData.IaInfoEvent                @R_IaInfoEvent      ;#asynchronous call
...
#
# Call Decline to inform that all the Ipv6 address that assigned have already been used by other nodes
#
...
while {${count} < 10} {
  ...
  ReceiveCcbPacket CCB DeclinePacket 5
  set assert pass
  if { ${CCB.received} == 0} {    
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid \
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Dhcp6DeclineFunc3AssertionGuid001 \
                "Dhcp6.Decline- Inform that all Ipv6 addresses assigned have already been used by other nodes" \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# Check point: After the Decline exchange process returns,the IaInfoEvent will be signaled and \
...
RecordAssertion $assert  $Dhcp6DeclineFunc3AssertionGuid002 \
...
RecordAssertion $assert  $Dhcp6DeclineFunc3AssertionGuid003 \
                "When Decline-Reply process finished, the state of IA should be Dhcp6Init"
