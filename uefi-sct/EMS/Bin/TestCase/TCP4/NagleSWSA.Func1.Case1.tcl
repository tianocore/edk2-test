...
#
CaseGuid          2E77B285-B8A8-49b6-BC14-7DC3A7DF8731
CaseName          NagleSWSA.Func1.Case1
CaseCategory      TCP
...
BeginLogPacket NagleSWSA.Func1.Case1   "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
# Nagle is disabled, so the 2nd "small" segment should be sent out immediately.
#
ReceiveTcpPacket TCB 2
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $Tcp4NagleSWSAFunc1AssertionGuid001                \
                    "EUT doesn't send out the 2nd data segment correctly."
    CleanUpEutEnvironment
    return
  } else {
      if { ${TCB.r_seq} != 2 || ${TCB.r_ack} != 1} {
        set assert fail
        RecordAssertion $assert $Tcp4NagleSWSAFunc1AssertionGuid001            \
                        "The sequence number or acknowledge number of the 2nd  \
                         data segment is not correct."
        CleanUpEutEnvironment
        return
      }
      if { ${TCB.r_len} != 1 } {
       	set assert fail
        RecordAssertion $assert $Tcp4NagleSWSAFunc1AssertionGuid001            \
                        "The data length of the 2nd data segment is not correct."
        CleanUpEutEnvironment
        return
      }
    }
