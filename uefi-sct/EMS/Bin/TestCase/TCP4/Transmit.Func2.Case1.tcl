...
CaseDescription   {This case is to test the Functionality.                     \
                  --Use Tcp4->Transmit() to transmit two fragments.}
################################################################################
...
BeginLogPacket Transmit.Func2.Case1 "host $DEF_EUT_IP_ADDR and host            \
                                             $DEF_ENTS_IP_ADDR"
#
...
SetVar R_FragmentBuffer1         "AAAABBBB"
...
SetVar R_TxData.DataLength                [expr $L_FragmentLength + $L_FragmentLength1]
...
SetVar R_TxData.FragmentTable(1)          @R_FragmentTable1
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4TransmitFunc2AssertionGuid001                     \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# [OS] gets the transmitted data packet.
#
ReceiveTcpPacket TCB 5

if { ${TCB.received} == 1 } {
  if { 536 > [ expr $L_FragmentLength + $L_FragmentLength1 ] } {
    if { ${TCB.r_f_ack} != 1 || ${TCB.r_len} != $L_FragmentLength + $L_FragmentLength1 } {
      RecordAssertion fail "EUT fail to transmit a data segment."
    } else {
      #
