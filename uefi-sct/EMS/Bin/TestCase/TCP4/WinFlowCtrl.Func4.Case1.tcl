...
#
CaseGuid          5B4BDF54-2070-4f06-8CFB-DCBF789883DA
CaseName          WinFlowCtrl.Func4.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles a link     \
                   ...
                   case test Right Edge Shrinks with Left Edge Closes.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func4.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o MssOption -w 4096
SendTcpPacket TCB
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc4AssertionGuid001                  \
...
#
# configure the [OS] to send back an ACK segment to acknowledge the data
# segments and change the advertised window to be 1024 octets and capture the
# responded segments
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_seq} != 1025 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT should not send out more data segments than the ENTS  \
                     window can handle at the time."
