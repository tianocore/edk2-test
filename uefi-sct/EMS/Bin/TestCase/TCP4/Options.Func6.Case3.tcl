...
CaseName          Options.Func6.Case3
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This item is to test TCP could correctly recognize and   \
                   deal with the timestamp option when it is used in TCP option.}
################################################################################
...
BeginLogPacket Options.Func8.Case3 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
CreatePayload  Option_MSS_WS_TS  Data  20                                      \
                                       ...
                                       0x01  0x01  0x08  0x0A                  \
                                       $TSval_3  $TSval_2  $TSval_1  $TSval_0  \
                                       0x00  0x00  0x00  0x00
...
CreatePayload  Option_TS  Data  12                                      \
                                0x01  0x01  0x08  0x0A                  \
                                $TSval_3  $TSval_2  $TSval_1  $TSval_0  \
                                $EUT_TSval_3  $EUT_TSval_2  $EUT_TSval_1  $EUT_TSval_0
...
while {1} {
  ...
  if { ${R_Receive_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Receive_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      ...
      RecordAssertion $assert $Tcp4OptionsFunc6AssertionGuid003                \
                      "Transmission failed."                                   \
