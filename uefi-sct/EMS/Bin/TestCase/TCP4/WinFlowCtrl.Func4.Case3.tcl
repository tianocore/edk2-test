...
#
CaseGuid          59A4A361-ECCC-45d9-9D89-84800743D9EB
CaseName          WinFlowCtrl.Func4.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles a link     \
                   partner's shrinking window with right-edge shrinking and    \
                   left-edge closing. A TCP receiver should not shrink the     \
                   window (move the right window edge to the left), but a      \
                   sending TCP must be robust against window shrinking. This   \
                   case test Right Edge Shrinks with Duplicated ACK.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func4.Case4 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc4AssertionGuid003                  \
...
#
# configure the [OS] to send back an ACK segment to acknowledge the SYN
# segment sent by the [EUT] and change the advertised window to be 2048 octets
...
UpdateTcpSendBuffer TCB -c $ACK -w 512
SendTcpPacket TCB

#
# window update segment including duplicated ACKs should be discarded.
#
set temp_ack [expr ${TCB.r_isn} + 1]
UpdateTcpSendBuffer TCB -c $ACK -w 1024 -a $temp_ack
SendTcpPacket TCB

ReceiveTcpPacket TCB 10
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
