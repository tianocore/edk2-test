...
#
CaseGuid          8E85ED81-6F36-4c8a-96F3-4E57D8A0C409
CaseName          CnntClosing.Func1.Case7
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the half-  \
                   close of the communication peer. If peer still want to send \
                   data after sending out FIN, EUT should ignore the data and  \
                   interact with the peer correctly.}
################################################################################
...
BeginLogPacket CnntClosing.Func1.Case7 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o MssOption -w 512
SendTcpPacket TCB
...
#
# OS get the transmitted data packet and respond with FIN, ACK segment to end
# one side of the connection. EUT should support this kind of half-close.
#
ReceiveTcpPacket TCB 5
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -w 0
SendTcpPacket TCB

#
# Expand the receive window together with data in the segment, EUT should      \
# ignore the data and interact with the peer correctly.
#
ReceiveTcpPacket TCB 5
UpdateTcpSendBuffer TCB -c $ACK -w 512 -p BoundaryPayload
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    ...
    return
      ...
      if { ${TCB.r_ack} != 2} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The acknowledge number of the first data segment is not correct."
  ...
  set assert pass
  RecordAssertion $assert $Tcp4CnntClosingFunc1AssertionGuid007                \
