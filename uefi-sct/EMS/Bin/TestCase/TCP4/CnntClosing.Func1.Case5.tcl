...
#
CaseGuid          5F5E8224-A974-4a65-A55E-3BE6A2638F43
CaseName          CnntClosing.Func1.Case5
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the        \
                   reception of unacceptable data segments after already       \
                   received partner's FIN segment.}
################################################################################
...
BeginLogPacket CnntClosing.Func1.Case5 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -a 0
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT should discard this unacceptable data segment, but not."
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntClosingFunc1AssertionGuid005                  \
