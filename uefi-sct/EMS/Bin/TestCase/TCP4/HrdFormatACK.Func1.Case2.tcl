...
#
CaseGuid          018C1B44-B16D-49c3-BF93-F35D51B39169
CaseName          HrdFormatAck.Func1.Case2
CaseCategory      TCP
CaseDescription   {This case is to test [EUT] correctly generates the ACK      \
                   numbers,and properly roll over the numbers. }
################################################################################
...
proc CleanUpEutEnvironment {} {
  ...
  EndLogPacket
  EndScope _TCP4_RFC_HrdFormat_ACK_
  EndLog
}
...
BeginScope _TCP4_RFC_HrdFormat_ACK_
BeginLogPacket HrdFormatACK.Conf1.Case2 "host $DEF_EUT_IP_ADDR and host  \
                                               $DEF_ENTS_IP_ADDR"
...
EFI_TCP4_CLOSE_TOKEN             R_CLOSE_TOKEN
...
#
# CheckPoint: Send Segment with seq 4294967294 to see EUTS whether retrun      \
#             rollover ack
#

set L_TCPFLage [expr $PSH]
CreatePayload  Payload0 String 3 "Ha"
UpdateTcpSendBuffer TCB -c $L_TCPFlage -p Payload0
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} !=1 } {
    set assert fail
    puts "EUT doesn't checksum and send out require ACK segment correctly."
    RecordAssertion $assert $Tcp4HrdFormatAckFunc1AssertionGuid002             \
                    "EUT doesn't checksum and send out ACK segment correctly."
    CleanUpEutEnvironment
    return
  } else {
      if {${TCB.r_ack} != 4 || ${TCB.r_seq} != 1} {
     	set assert fail
     	puts "EUT doesn't send the require  ACK segment correctly"
     	RecordAssertion $assert $Tcp4HrdFormatAckFunc1AssertionGuid002           \
     	                "EUT doen't checksum and send out ACK Segment ${TCB.r_ack}\
     	                 ${TCB.r_seq} correctly."
      	CleanUpEutEnvironment
      	return
}
