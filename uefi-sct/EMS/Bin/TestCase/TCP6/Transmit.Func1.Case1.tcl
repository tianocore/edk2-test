...
#
CaseGuid           5A1AF925-087B-45c4-8BA0-E95C1424FBF4
CaseName           Transmit.Func1.Case1
CaseCategory       TCP6
CaseDescription   {Transmit must succeed with valid parameters to transmit data.}
################################################################################
...
proc CleanUpEUTEnvironmentEnd {} {
  ...
  DestroyPacket
  EndCapture
  EndScope _TCP6_TRANSMIT_FUNC1_CASE1_
  EndLog
}
...
# BeginScope
#
BeginScope _TCP6_TRANSMIT_FUNC1_CASE1_
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid001                     \
...
if { ${CCB.received} == 0} {    
    set assert fail
    RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid002                \
...
RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid002                \
                "TCP6.Transmit - Packet is transmitted."
...
while {1 > 0} {
  ...
  if { $R_Context == 1} {
    ...
    set assert fail
    RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid003                            \
...
RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid003                            \
                "Transmit succeeded.(event has been signaled in time)."      \
                "TIMEOUT value is $L_TimeOut (sec), "

GetVar R_Transmit_IOToken.CompletionToken.Status
if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid004        \
...
set assert pass
RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid004    \
                        "Transmit succeeded(event status is modified correctly)."     \
                        "ReturnStatus - ${R_Transmit_IOToken.CompletionToken.Status},    \
                        ExpectedStatus - $EFI_SUCCESS"
...
if {[string compare $L_Flag $ACK] != 0} {
  set assert fail
  RecordAssertion $assert $Tcp6TransmitFunc1AssertionGuid005  \
