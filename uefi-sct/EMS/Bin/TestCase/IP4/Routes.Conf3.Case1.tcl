...
#
CaseGuid        6519F53B-D02B-46c2-BB27-7092F84A5154
CaseName        Routes.Conf3.Case1
CaseCategory    IP4
CaseDescription {Test the Routes Conformance of IP4 - Invoke Routes() to delete\
                 a route when it is not in the routing table. The return status\
...
# BeginScope
#
BeginScope _IP4_ROUTES_CONFORMANCE3_CASE1_
...
Ip4->Routes {TRUE, &@R_SubnetAddress, &@R_SubnetMask, &@R_GatewayAddress, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Ip4.Routes - Conf - Delete route"                             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

Ip4->Routes {TRUE, &@R_SubnetAddress, &@R_SubnetMask, &@R_GatewayAddress, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_NOT_FOUND]
RecordAssertion $assert $Ip4RoutesConf3AssertionGuid001                        \
                "Ip4.Routes - Conf - delete route again"                       \
...
# EndScope
#
EndScope _IP4_ROUTES_CONFORMANCE3_CASE1_
