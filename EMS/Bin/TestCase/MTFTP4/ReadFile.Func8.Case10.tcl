...
#
CaseGuid        BAAF90FC-2F4A-45ae-B828-04206F1BB8BD
CaseName        ReadFile.Func8.Case10
CaseCategory    MTFTP4
CaseDescription {This case is to test the Function of Mtftp4.ReadFile, testing \
                 the passive client's download while missing the first and the \
                 last packet. after client is timeout, server sets client to be\
                 passive again and sends out all the packets.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_READFILE_FUNCTION8_CASE10_
  EndLog
}
...
BeginScope _MTFTP4_READFILE_FUNCTION8_CASE10_
...
#
# If have captured the packet. Test the passive client's download while missing
# the first and the last packet. after client is timeout, server sets client to
# be passive again and sends out all the packets.
...
#
# After passive client is timeout, passive client should send ack0 to ask the 
# missing packet
#
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4ReadFileFuncAssertionGuid024                    \
                "Mtftp4.ReadFile - When the passive client is downloading, it  \
                 misses the first and the last packet. After client is timeout,\
                 server sets client to be passive again and sends out all the  \
                 packets."                                                     \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
