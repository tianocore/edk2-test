...
#
CaseGuid        64A017CC-5753-4ba5-B3EC-90CC5CCBE9F1
CaseName        ReadFile.Func8.Case12
CaseCategory    MTFTP4
CaseDescription {This case is to test the Function of Mtftp4.ReadFile - Test the\
                 passive client download, miss two blocks of packets. after    \
                  client timeout, server sets client to be passive again and   \
                  sends out the lost packets randomly.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_READFILE_FUNCTION8_CASE12_
  EndLog
}
...
BeginScope _MTFTP4_READFILE_FUNCTION8_CASE12_
...
#
  ...
  CleanUpEutEnvironmentg
  return
}

#
# If have captured the packet. Test the passive client download, miss two blocks
# of packets. after client timeout, server sets client to be passive again and 
# sends out the lost packets randomly.
...
SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 7 1024]
...
#
# Send block 4 will split the lost packets range
#
SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 4 1024]

#
# Send the start packet of one range
#
SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 2 1024]

#
# Send the last packet of one range
#
SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 6 1024]

SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 3 1024]

SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 5 1024]

SendPacket [ Mtftp4CreateData 1234 $EFI_MTFTP4_OPCODE_DATA 8 100]

GetAck

set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4ReadFileFuncAssertionGuid026                    \
                "Mtftp4.ReadFile - When the passive client downloads, it misses\
                 six blocks of packets. After client is timeout, server sets   \
                 client to be passive again and sends out the lost packets     \
                 randomly."                                                    \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
