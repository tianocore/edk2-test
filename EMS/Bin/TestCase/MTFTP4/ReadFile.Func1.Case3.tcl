...
#
CaseGuid        A6573EFE-D837-49d6-98D0-92A0D45318FB
CaseName        ReadFile.Func1.Case3
CaseCategory    MTFTP4
CaseDescription {This case is to test the Function of Mtftp4.ReadFile. Server  \
                 responses with normal OACK packet - after several times'      \
                 re-sending of client.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_READFILE_FUNCTION1_CASE3_
  EndLog
}
#
# Begin log ...
...
BeginScope _MTFTP4_READFILE_FUNCTION1_CASE3_
...
#
# receive a RRQ from the client, it should retry for 5 times.
#
    incr count  
  }
}
...
if {$count != 5} {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Mtftp4.ReadFile - client should retry for 5 times, but not."
}
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4ReadFileFuncAssertionGuid003                    \
                "Mtftp4.ReadFile - Server responses with normal OACK packet -  \
                 after several times' re-sending of client."                   \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
