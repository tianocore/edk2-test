...
#
CaseGuid        0E0A407C-840E-40b9-A16B-ED186E1BE9AF
CaseName        ReadFile.Func3.Case1
CaseCategory    MTFTP4
CaseDescription {This case is to test the Function of Mtftp4.ReadFile - After  \
                 passive client having received some packets, the server sets it\
                 to be active and sends remain packet.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_READFILE_FUNCTION3_CASE1_
  EndLog
}
...
BeginScope _MTFTP4_READFILE_FUNCTION3_CASE1_
...
#
# If have captured the packet. Server responses with OACK packet to set the 
# client passive.
#
...
SendPacket [ Mtftp4CreateData $client_prt $EFI_MTFTP4_OPCODE_DATA 1 1024] 

SendPacket [ Mtftp4CreateData $client_prt $EFI_MTFTP4_OPCODE_DATA 3 100] 

set option_value_array(multicast) ",,1"
set option_value_len(multicast) 3
...
SendPacket [ Mtftp4CreateData $client_prt $EFI_MTFTP4_OPCODE_DATA 2 1024] 
...
# the second parameter indicates the block number.
#
ValidateAck TempPacket4 3 

GetAck

set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4ReadFileFuncAssertionGuid006                    \
                "Mtftp4.ReadFile - After passive client having received some   \
                 packets, the server sets it to be active and sends remain     \
                 packet."                                                      \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
