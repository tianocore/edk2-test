...
#
CaseGuid        0D177831-4429-4d3d-96B5-BBF401770E75
CaseName        ReadFile.Func2.Case2
CaseCategory    MTFTP4
CaseDescription {This case is to test the function of Mtftp4.ReadFile - When   \
                 client receives an error server source port data packet, it   \
                 just ignores the packet and continues the data processing.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_READFILE_FUNCTION2_CASE2_
  EndLog
}
...
BeginScope _MTFTP4_READFILE_FUNCTION2_CASE2_
...
#
# If have captured the packet. Client receive an error server source port ACK 
# when waiting for ACK packet.
...
CreatePayload _block Data 2 0x00 0x01
CreatePayload _data  const 100 0x5a
ConcatPayload mtftp4_data_pld OpCode _block _data
CreatePacket  mtftp4_data -t udp -udp_sp 6666 -udp_dp 1234 -udp_payload        \
               mtftp4_data_pld
SendPacket mtftp4_data
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4ReadFileFuncAssertionGuid005                    \
                "Mtftp4.ReadFile - Client receive an error server source port  \
                 data packet."                                                 \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
