...
#
CaseGuid        9D90730C-B705-449e-A566-BE92F243A005
CaseName        WriteFile.Func2.Case2
CaseCategory    MTFTP4
CaseDescription {This case is to test the function of Mtftp4.WriteFile, Server \
                 responds WRQ with an ACK instead of OACK, so client sends data\
                 packet to server.}
################################################################################
...
#
  ...
  EndCapture
  EndScope _MTFTP4_WRITEFILE_FUNCTION2_CASE2_
  EndLog
}
...
BeginScope _MTFTP4_WRITEFILE_FUNCTION2_CASE2_
...
#
# If have captured the packet, Server responds WRQ with an ACK instead of OACK, 
# so client sends data packet to server.
#
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Mtftp4WriteFileFuncAssertionGuid005                   \
                "Mtftp4.WriteFile - Server responds WRQ with an ACK instead of \
                 OACK, so client sends data packet to server."                 \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
