...
#
CaseGuid          06871E09-3A9B-4d76-BFEA-3146A5473765
CaseName          Connect.Func1.Case1
CaseCategory      TCP6
CaseDescription   {Connect must succeed with valid parameters in active mode.}
################################################################################
...
proc CleanUpEUTEnvironmentEnd {} {
  ...
  DestroyPacket
  EndCapture
  EndScope _TCP6_CONNECT_FUNC1_CASE1_
  EndLog
}
...
# BeginScope
#
BeginScope _TCP6_CONNECT_FUNC1_CASE1_
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid001               \
                "Tcp6.Connect - Open an active connection with correct parameters."      \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { ${CCB.received} == 0} {    
  set assert fail
  RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid002                \
...
set assert pass
RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid002                \
                "TCP6.Connect - SYN is sent successfully."
...
if { ${CCB.received} == 0} {    
  set assert fail
  RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid003                \
...
RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid003  \
                "Tcp6.Connect - ACK is sent out successfully."
...
while {1 > 0} {
  ...
  if { $R_Context == 1} {
    ...
    set assert fail
    RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid004                            \
                    "Three-way handshake for active connection failed (event hasn't been signaled before TIMEOUT)."      \
...
set assert pass
RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid004                            \
                "Three-way handshake for active connection succeeds (event has been signaled in time)."      \
                "TIMEOUT value is $L_TimeOut (sec), "

GetVar R_Connect_ConnectionToken.CompletionToken.Status
if { ${R_Connect_ConnectionToken.CompletionToken.Status} != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid005        \
                  "Three-way handshake for active connection failed(event status is not correct)."      \
...
set assert pass
RecordAssertion $assert $Tcp6ConnectFunc1AssertionGuid005        \
                "Three-way handshake for active connection success(Token Status is modified)."      \
                "ReturnStatus - ${R_Connect_ConnectionToken.CompletionToken.Status},           \
                ExpectedStatus - $EFI_SUCCESS"
#
# EUT ends the TCP session initiatively
#
EUTEndSession
