...
#
CaseGuid        7821F197-2C95-4482-932F-93D30D818181
...
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Arp.Request - Send request again"                             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS,      \
                 Context - $R_EventContext, ExpectedContext - 1"
...
GetAck
#
# Stall to make the entry timeout.
#
Stall 20
...
puts $R_TargetHwAddress
SetIpv4Address R_TargetSwAddress.v4 "172.16.210.161"
Arp->Request {&@R_TargetSwAddress, @R_ResolvedEvent2, &@R_TargetHwAddress, &@R_Status}

GetAck
set assert [VerifyReturnStatus R_Status $EFI_NOT_READY]

RecordAssertion $assert $GenericAssertionGuid                                  \
                "Arp.Request - Send request again after entry timeout"         \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_NOT_READY"
...
RecordAssertion $assert $ArpConfigureFuncAssertionGuid003                      \
                "Arp.Request - R_TargetHwAddress is correct"                   \
