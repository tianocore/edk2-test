...
#
CaseGuid        CF70B806-80B7-41b0-A24D-2EF0776DFB83
CaseName        Add.Func1.Case2
CaseCategory    ARP
CaseDescription {This case is to test the function of ARP.Add - when overwrite \
	               is true, the match entry cache is overwritten by another      \
	               TargetHwAddress.}
################################################################################
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Arp.Add - Add deny entry"                                     \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
#
# Check point
#
SetEthMacAddress R_TargetHwAddress "06:05:04:03:02:01"
Arp->Add {FALSE, &@R_TargetSwAddress, &@R_TargetHwAddress, 0, TRUE, &@R_Status}
GetAck

set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $ArpAddFuncAssertionGuid002                            \
                "Arp.Add - Add normal entry overwrite"                         \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if {[string compare -nocase $R_TargetHwAddress 06:05:04:03:02:01] == 0} {
  set assert pass
} else {
  set assert fail
}
RecordAssertion $assert $GenericAssertionGuid                                  \
                ...
                "Get - $R_TargetHwAddress, Expected - 06:05:04:03:02:01"
