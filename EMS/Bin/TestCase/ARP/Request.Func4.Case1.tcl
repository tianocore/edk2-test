...
#
CaseGuid        76499EFB-2129-478d-9385-F3D6F7CA95A8
CaseName        Request.Func4.Case1
CaseCategory    ARP
...
SetIpv4Address R_TargetSwAddress.v4 "255.255.255.255"

#
# Check point
#
Arp->Request {NULL, @R_ResolvedEvent, &@R_TargetHwAddress, &@R_Status}
...
RecordAssertion $assert $ArpRequestFuncAssertionGuid006                        \
                "Arp.Request - Request the entry when TargetSwAddress is       \
                 255.255.255.255"                                              \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS,      \
                 Context - $R_EventContext, ExpectedContext - 1"

set R_TargetHwAddress [GetEthMacAddress R_TargetHwAddress]
if {[string compare -nocase $R_TargetHwAddress ff:ff:ff:ff:ff:ff] == 0} {
  set assert pass
} else {
  set assert fail
}
RecordAssertion $assert $GenericAssertionGuid                                  \
                ...
                "Get - $R_TargetHwAddress, Expected - ff:ff:ff:ff:ff:ff"
...
RecordAssertion $assert $ArpRequestFuncAssertionGuid007                        \
