...
#
CaseGuid        5962b61d-04fd-4dad-af25-3f2ef9982051
CaseName        Stop.Func2.Case1
CaseCategory    DHCP4
CaseDescription {This case is to test the Functionality.                       \
	              -- Call Stop in Configuration Process.}
...
#
# Respond to DHCP Client in the following message sequence
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Dhcp4.Stop - Func - Start configuration process."             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { ${CCB.received} == 0 } {
  set assert fail
	RecordAssertion $assert $Dhcp4StopFunc2AssertionGuid001                      \
                  "Dhcp4.Start - No DHCPREQUEST packet Captured."

  CleanUpEutEnvironment
  return
} else {
  ParsePacket RequestPacket -t dhcp -dhcp_xid xid -dhcp_chaddr chaddr          \
              -dhcp_hlen hlen -dhcp_htype htype -dhcp_options options
...
RecordAssertion $assert $GenericAssertionGuid                                  \
                ...
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"      \
                "CurState - ${R_ModeData.State}, ExpectState - $Dhcp4Rebooting \
                CurClientAddress - $tempaddr, ExpectClientAddr - 192.168.2.4."

#
# 2). Respond with DHCPOFFER message
#
LocalEther   0:3:5:6:6:7
RemoteEther  $eth_src
LocalIp      192.168.2.1

CreateDhcpOpt ack1_opt1 mesg_type          2;    #Message Type = DHCPOFFER
...
#
# Check Point: Call [DHCP4] -> Stop before DHCP4 configuration process finishes.
# ClientAddress will set to 0.0.0.0
#
Dhcp4->Stop  "&@R_Status"
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Dhcp4StopFunc2AssertionGuid001                        \
                "Dhcp4.Stop - Func - Stop Driver In config process."           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { [string compare -nocase $assert "pass"] == 0 } {
  ...
  if { ${R_ModeData.State} != $Dhcp4Stopped } {
    ...
    if { [string compare -nocase $tempaddr "0.0.0.0"] != 0 } {
      set assert fail
    }
  }
}
...
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Dhcp4.Start - Func - Check the DHCP instance State"           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"      \
                "CurState - ${R_ModeData.State}, ExpectState - $Dhcp4Stopped   \
                CurClientAddress - $tempaddr, ExpectClientAddr - 0.0.0.0."
