...
#
CaseGuid        052D37EA-63D7-4118-B446-7830D5BCE670
CaseName        RenewRebind.Func1.Case1
CaseCategory    DHCP6

CaseDescription {Test the RenewRebind Function of DHCP6 - Invoke RenewRebind() \
                 to Manually extend the valid and preferred lifetime for the \
                  IPv6 addresses of the configured IA and update other configuration
                }
################################################################################
...
# EndScope
#
EndScope _DHCP6_RENEWREBIND_FUNC1_
#
# End Log 
#
EndLog
...
# BeginScope
#
BeginScope  _DHCP6_RENEWREBIND_FUNC1_
...
#
# Check point:Call RenewRebind,the exchange process includes RENEW-REPLY
#
BOOLEAN                  R_RebindRequest 
SetVar  R_RebindRequest  FALSE
Dhcp6->RenewRebind  "@R_RebindRequest, &@R_Status"

set count 0

while {${count} < 10} {

  ReceiveCcbPacket CCB RenewRebindPacket 5
  set assert pass
  if { ${CCB.received} == 0} {    
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid \
                    "Dhcp6.RenewRebind- No Renew Packet sent."
    CleanUpEnvironment
    return
  }

  ParsePacket RenewRebindPacket -t IPv6 -IPv6_src IPV6Src -IPv6_payload IPV6Payload
...
set ReplyPayloadData     [CreateDHCP6RenewReplyPayloadData $IPV6Src $LocalIP $TransId_and_ClientId]
...
CreatePayload RenewReplyPayload List $PayloadDataLength $ReplyPayloadData

#
# Create RenewReply packet
#
CreatePacket RenewReplyPacket -t IPv6 -IPv6_ver 0x06 -IPv6_tc 0x00 -IPv6_fl 0x00 \
                              -IPv6_pl $PayloadDataLength -IPv6_nh 0x11 -IPv6_hl 128 \
                              -IPv6_payload RenewReplyPayload

SendPacket RenewReplyPacket

#
# Get Ack of RenewRebind()
#
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Dhcp6RenewRebindFunc1AssertionGuid001 \
                "Dhcp6.RenewRebind - Call RenewRebind to finish the process of RENEW-REPLY"\
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
#
# Check point: After the RENEW-REPLY process,Callback() has been called twice,\
#              so CallbackContext should add 2,and the state is still Dhcp6Bound.
#
GetVar R_CallbackContext
set assert pass
if { $R_CallbackContext != 7 } {
  set assert fail
  RecordAssertion $assert $Dhcp6RenewRebindFunc1AssertionGuid002 \
                  "Dhcp6.RenewRebind- when Dhcp6Event occurs,Callback function will be called."
}
...
RecordAssertion $assert  $Dhcp6RenewRebindFunc1AssertionGuid003 \
                "When RENEW-REPLY process finished, the state of IA should be Dhcp6Bound"
