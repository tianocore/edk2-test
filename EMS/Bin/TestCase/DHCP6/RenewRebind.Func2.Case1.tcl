...
#
CaseGuid        C302C3F8-EDD7-4650-B4B3-E3669EB2886C
...
# EndScope
#
EndScope _DHCP6_RENEWREBIND_FUNC2_
#
# End Log 
#
EndLog
...
# BeginScope
#
BeginScope  _DHCP6_RENEWREBIND_FUNC2_
...
BS->CopyMem  "&@R_Ia, @R_Dhcp6ModeData1.Ia, @R_CopyLen, &@R_Status"
GetAck
...
#
# Check point:Call RenewRebind to send Renew packet while no response from server.
#
...
while {${count} < 10} {
  ...
  ReceiveCcbPacket CCB RenewPacket 5
  set assert pass
  if { ${CCB.received} == 0} {    
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid \
  ...
  ParsePacket RenewPacket -t IPv6 -IPv6_src IPV6Src -IPv6_payload IPV6Payload
...
while {${count} < 10} {
  ...
  ReceiveCcbPacket CCB RebindPacket 14
  set assert pass
  if { ${CCB.received} == 0} {    
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid \
                    "Dhcp6.RenewRebind- No Rebind Packet sent."
    CleanUpEnvironment
    return
  }

  ParsePacket RebindPacket -t IPv6 -IPv6_src IPV6Src -IPv6_payload IPV6Payload
  set MessageType [lrange $IPV6Payload 8 8]
...
#
CreatePacket RebindReplyPacket -t IPv6 -IPv6_ver 0x06 -IPv6_tc 0x00 -IPv6_fl 0x00 \
                               -IPv6_pl $PayloadDataLength -IPv6_nh 0x11 -IPv6_hl 128 \
                               -IPv6_payload RenewReplyPayload

SendPacket RebindReplyPacket
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Dhcp6RenewRebindFunc2AssertionGuid001 \
                "Dhcp6.RenewRebind - When time reached T2,Rebind packet was sent out"\
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
#
# Check point:After RenewRebind returns the state of IA should be Dhcp6Bound.
#
...
RecordAssertion $assert  $Dhcp6RenewRebindFunc2AssertionGuid002 \
                "When RenewRebind process finished, the state of IA should be Dhcp6Bound"
CleanUpEnvironment
