...
#
CaseGuid        A0DBA98B-E8D0-46d7-ACAB-35C9F3AAAD95
CaseName        Start.Conf3.Case1
CaseCategory    DHCP6

CaseDescription {Test the Start Conformance of DHCP6 - Invoke Start() \
                 when IaInfoEvent is NULL and Dhcp6 server doesn't response. \
                 After SolicitRetransmission timeout. EFI_NO_RESPONSE should be returned.
                }
################################################################################
...
# BeginScope
#
BeginScope  _DHCP6_START_CONF3_
...
#
# Check point: Call Start() with IaInfoEvent NULL when the DHCP6 server doesn't response
#              EFI_NO_RESPONSE should be returned.
#
Dhcp6->Start "&@R_Status"
GetAck
set assert [VerifyReturnStatus R_Status $EFI_NO_RESPONSE]
RecordAssertion $assert $Dhcp6StartConf3AssertionGuid001                   \
                "Dhcp6.Start - Dhcp6 server doesn't response"                       \
