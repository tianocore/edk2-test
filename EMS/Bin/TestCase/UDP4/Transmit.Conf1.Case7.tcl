...
#
CaseGuid        0DFF854F-4D95-48f7-9BF3-56BF841144EB
CaseName        Transmit.Conf1.Case7
CaseCategory    Udp4
CaseDescription {Test the Transmit Conformance of UDP4 - Invoke Transmit() with\
                 the parameter Token.Packet.TxData.GatewayAddress being not a  \
                 valid unicast IPv4 address if it is not NULL. The return status\
...
# BeginScope
#
BeginScope _UDP4_TRANSMIT_CONFORMANCE1_CASE7_
...
SetIpv4Address R_GatewayAddress                 "255.255.255.255"
...
RecordAssertion $assert $Udp4TransmitConf1AssertionGuid008                     \
                "Udp4.Transmit - Conf - With the parameter                     \
                 Token.Packet.TxData.GatewayAddress being not a valid unicast  \
                 IPv4 address if it is not NULL. "                             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_INVALID_PARAMETER"

SetIpv4Address R_GatewayAddress                 "172.16.220.255"
SetVar         R_TxData.GatewayAddress           &@R_GatewayAddress

Udp4->Transmit {&@R_Token, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_INVALID_PARAMETER]
RecordAssertion $assert $Udp4TransmitConf1AssertionGuid009                     \
                "Udp4.Transmit - Conf - With the parameter                     \
                 Token.Packet.TxData.GatewayAddress being not a valid unicast  \
                 IPv4 address if it is not NULL. "                             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_INVALID_PARAMETER"
              
SetIpv4Address R_GatewayAddress                  "224.0.0.2"
SetVar         R_TxData.GatewayAddress           &@R_GatewayAddress

Udp4->Transmit {&@R_Token, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_INVALID_PARAMETER]
RecordAssertion $assert $Udp4TransmitConf1AssertionGuid010                     \
                "Udp4.Transmit - Conf - With the parameter                     \
                 Token.Packet.TxData.GatewayAddress being not a valid unicast  \
                 IPv4 address if it is not NULL. "                             \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_INVALID_PARAMETER"
...
# EndScope
#
EndScope _UDP4_TRANSMIT_CONFORMANCE1_CASE7_
