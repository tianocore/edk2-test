...
CaseDescription   {This case is to test the Functionality.                     \
                   ...
                   -- format 1:                                                \
                     0            7           15           23           31     \
                     +------------+------------+------------+------------+     \
                     |   No-Oper  |  00000010  |  00000100  |     max    |     \
                     +------------+------------+------------+------------+     \
                     +------------+------------+------------+------------+     \
                     |  seg size  |   No-Oper  |   No-Oper  |   No-Oper  |     \
                     +------------+------------+------------+------------+     \
                  .}
################################################################################
...
BeginLogPacket Options.Func6.Case8 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
#
CreatePayload OptionMSS Data 8 0x01 0x02 0x04 $L_MSSH $L_MSSL 0x01 0x01 0x01
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc4AssertionGuid008                \
