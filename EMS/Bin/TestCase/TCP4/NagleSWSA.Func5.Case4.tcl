...
#
CaseGuid          E7467BDD-5B12-4ead-8D09-7AC2E85512A3
CaseName          NagleSWSA.Func5.Case4
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly avoids the Silly   \
                   ...
                   transmit unless at least one-half of the Max Window that    \
                   receive ever advertised.}
################################################################################
...
BeginLogPacket NagleSWSA.Func6.Case4   "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o MssOption -w 518
SendTcpPacket TCB
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4NagleSWSAFunc5AssertionGuid004                    \
...
UpdateTcpSendBuffer TCB -c $ACK -w 6
SendTcpPacket TCB

#
# EUT should set persist timer, configure OS to increase the window size by
# 256 octets consecutively. Make sure the windows size exceed one-half of the
# Max Window that receive ever advertised before persist timer times out.
#
UpdateTcpSendBuffer TCB -c $ACK -w 262
SendTcpPacket TCB
...
#
# Increase the windows size step by step, when it accesses the left data size,
# EUT should send out the left buffered data at one time.
#
UpdateTcpSendBuffer TCB -c $ACK -w 0
SendTcpPacket TCB

UpdateTcpSendBuffer TCB -c $ACK -w 258
SendTcpPacket TCB

UpdateTcpSendBuffer TCB -c $ACK -w 426
SendTcpPacket TCB
...
UpdateTcpSendBuffer TCB -c $ACK -w 518
SendTcpPacket TCB
