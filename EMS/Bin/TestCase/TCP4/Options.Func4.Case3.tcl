...
#
CaseGuid          22209BBC-723C-41ae-9E95-DE017E1453A4
CaseName          Options.Func4.Case3
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This item is to test the [EUT] correctly adheres to the  \
                   MSS of the connection.                                      \
                   -- [EUT] will automatically divide up transmitting data     \
                   segment if its size larger than [OS] announced MSS value.}
################################################################################
...
BeginLogPacket Options.Func6.Case3 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc4AssertionGuid003                \
