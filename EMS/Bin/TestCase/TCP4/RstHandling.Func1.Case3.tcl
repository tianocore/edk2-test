...
#
CaseGuid          2D02D005-7A6A-4d25-BB0D-0C9A44B2D776
CaseName          RstHandling.Func1.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly send out the reset \
                   segment while in CLOSE-WAIT state.}
################################################################################
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC1_CASE3_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC1_CASE3_

BeginLogPacket RstHandling.Func1.Case3 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "<ESTABLISHED>:OS->EUT:FIN,Expect:EUT->OS:ACK"             \
...
# Validate now TCP is in CLOSE_WAIT state
Tcp4->GetModeData {&@R_Connection_State, NULL, NULL, NULL, NULL, &@R_Status}
GetAck
GetVar R_Connection_State
if { $R_Status!=$EFI_SUCCESS || $R_Connection_State!=$Tcp4StateCloseWait } {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc1AssertionGuid003                \
                  "Active Connection, Enter CLOSE_WAIT"                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState-$R_Connection_State,ExpectedState-Tcp4StateCloseWait"
...
RecordMessage DEFAULT "Enter CLOSE_WAIT state"
...
set L_CorrectSeq ${TCB.l_next_seq}
set temp [expr ${TCB.l_isn} + $L_CorrectSeq + ${TCB.r_win} + 100]
UpdateTcpSendBuffer TCB -c $L_TcpFlag -a [expr ${TCB.r_isn} + $L_CorrectAck+${TCB.l_win}+200]
#UpdateTcpSendBuffer TCB -c $L_TcpFlag -a [expr ${TCB.r_isn}]
#UpdateTcpSendBuffer TCB -c $L_TcpFlag -s $temp -p HelloWorld
SendTcpPacket TCB
...
set TCB.l_next_seq $L_CorrectSeq
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "<ESTABLISHED>: OS -> EUT: Unaccept ACK, "                 \
...
#
if { [expr $L_OrigSeq-1] != ${TCB.r_seq} } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "<ESTABLISHED>: OS -> EUT: Unaccept ACK, "                   \
                  "Expect: EUT -> OS: ACK indicate correct seq,"               \
                  "ACK.ack_id = [expr $L_OrigSeq-1], Result: ${TCB.r_seq}"
...
#
# Send RST to disconnect the session
#
UpdateTcpSendBuffer TCB -c $RST; #RST
SendTcpPacket TCB
