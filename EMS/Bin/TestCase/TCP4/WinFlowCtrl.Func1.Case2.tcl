...
#
CaseGuid          C22B8C87-B5FD-4860-81FB-32739E8D0281
CaseName          WinFlowCtrl.Func1.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly receives the       \
                   segment that has the advertised receive window open         \
                   right-edge and keep left-edge.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func1.Case3 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o MssOption -w 1536
SendTcpPacket TCB
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc1AssertionGuid002                  \
...
#    segment and change the advertised window to be 1024 octets.
# 2. The second is to acknowledge the SYN segment sent from the [EUT] and
#    change the advertised window to be 1536 octets.
#
UpdateTcpSendBuffer TCB -c $ACK -w 1024
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the second data segment correctly."
    ...
    return
      ...
      if { ${TCB.r_seq} != 1025 || ${TCB.r_ack} != 1} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number or acknowledge number of the second\
                         data segment is not correct."
      ...
      if { ${TCB.r_len} != 1024 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the second data segment is not correct"
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the second data segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
...
set temp_ack [expr ${TCB.r_isn} + 1025]
UpdateTcpSendBuffer TCB -c $ACK -w 1536 -a $temp_ack
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the third data segment correctly."
    ...
    return
      ...
      if { ${TCB.r_seq} != 1025 || ${TCB.r_ack} != 1} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number or acknowledge number of the third\
                         data segment is not correct."
      ...
      if { ${TCB.r_len} != 1024 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the third data segment is not correct"
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the third data segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
...
UpdateTcpSendBuffer TCB -c $ACK -w 1536
SendTcpPacket TCB
