...
#
CaseGuid          3B85703D-2A2C-4e45-8AAD-FEECC4C6AC2B
CaseName          Transmit.Func1.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles FIN segment\
...
BeginLogPacket Transmit.Func1.Case1 "host $DEF_EUT_IP_ADDR and host            \
                                          $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4TransmitFunc1AssertionGuid001                     \
  ...
  if { ${TCB.r_f_ack} != 1 } {
    ...
    return
      ...
      if { ${TCB.r_len} != 512 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the first data segment is          \
                         not correct"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -w 256
SendTcpPacket TCB
...
UpdateTcpSendBuffer TCB -c $ACK -w 256
SendTcpPacket TCB
