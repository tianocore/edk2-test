...
#
CaseGuid          6EDEADFB-5870-4ae8-ADD8-EDECA0F4635E
...
BeginLogPacket GetModeData.Func3.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
SetVar R_Configure_Tcp4ConfigData.TimeToLive          $L_TimeToLive
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4GetModeDataFunc2AssertionGuid002                  \
...
if {${R_Tcp4ConfigData.TimeToLive} != $L_TimeToLive} {
  puts "Tcp4ConfigData.TimeToLive = ${R_Tcp4ConfigData.TimeToLive}\n"
  puts "L_TimeToLive - $L_TimeToLive\n"
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "TimeToLive was not the same that configured before."
}
