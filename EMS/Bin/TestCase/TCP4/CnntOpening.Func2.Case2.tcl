...
#
CaseGuid          EA8438FE-E33F-4f98-8A43-4BC08F92EF51
CaseName          CnntOpening.Func2.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the flag   \
                   combination: FIN, ACK through passive open.}
################################################################################
...
BeginLogPacket CnntOpening.Func2.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntOpeningFunc2AssertionGuid002                  \
...
#
# The data sent together with the FIN,ACK segment should be processed.
#
set L_TcpFlag [expr $ACK | $FIN | $PSH]
  ...
  if { ${TCB.r_f_ack} != 1 } {
    ...
    if { ${TCB.r_ack} != 514 } {
      ...
      RecordAssertion $assert $GenericAssertionGuid                            \
                              ...
                              is not correct, {TCB.r_ack} = ${TCB.r_ack}."
