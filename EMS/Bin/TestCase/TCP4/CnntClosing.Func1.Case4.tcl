...
#
CaseGuid          3F5709D4-8A5E-46cf-A2F6-BE47E59BC17D
CaseName          CnntClosing.Func1.Case4
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the        \
                   reception of normal data segments after already received    \
                   partner's FIN segment.}
################################################################################
...
ReceiveTcpPacket TCB 5
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_rst} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                            "EUT doesn't send out RST, ACK segment to respond  \
                            data segment correctly."
    CleanUpEutEnvironment
    return
  } else {
...
#
# Expectation: EUT should return to CLOSE state.
#
...
if { $R_Connection_State != $Tcp4StateClosed || $R_Status != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $Tcp4CnntClosingFunc1AssertionGuid004                \
