...
#
CaseGuid          86B4E4E5-A2A1-48d6-901F-6C7C9D31F8C2
CaseName          CongestionCtrl.Func1.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly performs the slow  \
                   start and congestion avoidance algorithms when data segment \
...
BeginLogPacket CongestionCtrl.Func1.Case2 "host $DEF_EUT_IP_ADDR and host      \
                                                $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $CongestionCtrlFunc1AssertionGuid002                   \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# Configure the OS to make data retransmission. EUT should re-enter the slow
# start after timeout. After the first stage of data transmission, the cwnd
# should be expanded to 11*SMSS.
#
for { set i 0} { $i < 11 } { incr i } {
  ReceiveTcpPacket TCB 1
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 || ${TCB.r_seq} != $expect_r_seq || ${TCB.r_len} != $expect_r_len} {
      set assert fail
      RecordAssertion $assert $GenericAssertionGuid                            \
                      "The sequence number or acknowledge number or flag of the\
...
#
# Wait for data retransmission and send back the ACK to all the transmitted
# data segments.
#
UpdateTcpSendBuffer TCB -c $ACK
Stall 5
...
#
# The EUT should enter slow start, check it.
#
while { 0 < 1} {
    ...
    if { ${TCB.r_f_ack} != 1 } {
      ...
      return
        ...
        if { ${TCB.r_seq} != $expect_r_seq || ${TCB.r_ack} != 1} {
          set assert fail
          RecordAssertion $assert $GenericAssertionGuid                        \
                          "The sequence number or acknowledge number           \
                           of the data segment is not correct.                 \
                           expect_r_seq = $expect_r_seq, captured = ${TCB.r_seq}"
          CleanUpEutEnvironment
          return
        }
        if { ${TCB.r_len} != $expect_r_len } {
