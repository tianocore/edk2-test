...
#
CaseGuid          19FCB41E-B734-46b1-9E9E-A9383B56E168
CaseName          Options.Func6.Case4
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   ...
                   -- format 2:                                                \
                     0            7           15           23           31     \
                     +------------+------------+------------+------------+     \
                     |  00000100  |  00000110  |    TS Value (TSval)     |     \
                     +------------+------------+------------+------------+     \
                     +------------+------------+------------+------------+     \
                     |         (TSval)         |  TS Echo Reply (TSecr)  |     \
                     +------------+------------+------------+------------+     \
                     +------------+------------+------------+------------+     \
                     |         (TSecr)         |   No-Oper  |   No-Oper  |     \
                     +------------+------------+------------+------------|.}
################################################################################
...
BeginLogPacket Options.Func8.Case9 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
CreatePayload  Option_TS  Data  12                                             \
                                0x08  0x0A  $TSval_3  $TSval_2                 \
                                $TSval_1  $TSval_0  $EUT_TSval_3  $EUT_TSval_2 \
                                $EUT_TSval_1 $EUT_TSval_0 0x01  0x01
...
while {1} {
  ...
  if { ${R_Receive_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Receive_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      ...
      RecordAssertion $assert $Tcp4OptionsFunc6AssertionGuid004                \
                      "Transmission failed."                                   \
