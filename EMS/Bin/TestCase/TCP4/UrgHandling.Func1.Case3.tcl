...
#
CaseGuid          422C6775-5A7A-44f3-B7DA-D3064EEFCE02
CaseName          UrgHandling.Func1.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly uses the urgent    \
                   ...
                   urgent pointer rollback for two times.}
################################################################################
...
BeginLogPacket CongestionCtrl.Func1.Case3 "host $DEF_EUT_IP_ADDR and host      \
                                                $DEF_ENTS_IP_ADDR"
...
#
# 1456 = 1460 (RFC_TCP_MAX_MSS) - 4 (VLAN tag)
# 131041 = 1456*90 + 1
set    L_FragmentLength          131041
...
#
# Set MSS to be 1456 (1460 - 4), this allows EUT in VLAN envrionment
#
BuildTcpOptions MssOption -m 1456
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4UrgHandlingFunc1AssertionGuid003                                  \
...
#
set expect_r_seq 1
set expect_r_len 1456
set actual_r_urg 131040
set expect_r_urg 65535
  ...
  for { set j 0} { $j < $expect_data_seg_num } { incr j } {
    ...
    if { ${TCB.received} == 1 } {
      ...
      if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_urg} != 1} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "EUT doesn't send out the urgent data segment correctly."
        ...
        return
          ...
          if { ${TCB.r_len} != $expect_r_len || ${TCB.r_urg} != $expect_r_urg } {
            set assert fail
            RecordAssertion $assert $GenericAssertionGuid                      \
                            "The data length of the data segment is not correct,\
                             {TCB.r_len} = ${TCB.r_len} expect_r_len = $expect_r_len, \
                             {TCB.r_urg} = ${TCB.r_urg} expect_r_urg = $expect_r_urg."
          ...
          set actual_r_urg [expr $actual_r_urg - $expect_r_len]
          if {$actual_r_urg < 65535} {
            set expect_r_urg $actual_r_urg
            if {$actual_r_urg < 1456} {
              set expect_r_len 1
            }
          }
        }
