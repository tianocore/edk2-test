...
CaseGuid          A288BB1C-D7B6-4194-8D4C-C52638499B9A
CaseName          RetransmissionTimer.Func1.Case1
CaseCategory      TCP
CaseDescription   {This case is to test the [EUT] correctly retransmission timer\
                   with the method of exponential backoff. }
################################################################################
...
BeginLogPacket Timer.Retransmission.Func1.case1 "host $DEF_EUT_IP_ADDR and host\
                                               $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4RetransmissionTimerFunc1AssertionGuid001          \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { ${TCB.received} == 1 } {
   ...
   set assert fail
   puts "EUT do not send enough retransmission segment and close connection."
   RecordAssertion $assert $GenericAssertionGuid                               \
                   "EUT do not send enough retransmission segment and close connection. "
