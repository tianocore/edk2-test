...
#
CaseGuid          2FD8730A-C379-4c3e-956A-05F9BD2F7530
CaseName          HrdFormatAck.Func1.Case1
CaseCategory      TCP
...
BeginLogPacket HrdFormatACK.Func1.Case1 "host $DEF_EUT_IP_ADDR and host  \
                                               $DEF_ENTS_IP_ADDR"
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} !=1 } {
    set assert fail
    puts "EUT doesn't send out require ACK segment correctly."
    RecordAssertion $assert $Tcp4HrdFormatAckFunc1AssertionGuid001             \
                    "EUT doesn't send out ACK segment correctly."
    CleanUpEutEnvironment
    return
  } else {
      if {${TCB.r_ack} != 4 || ${TCB.r_seq} != 1} {
     	...
     	RecordAssertion $assert $Tcp4HrdFormatAckFunc1AssertionGuid001           \
     	                "EUT doen't send out ACK Segment ${TCB.r_ack} ${TCB.r_seq}\
     	                 correctly."
      	CleanUpEutEnvironment
      	return
      	}
