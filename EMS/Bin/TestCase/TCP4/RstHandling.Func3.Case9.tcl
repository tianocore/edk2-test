...
#
CaseGuid          1D50C4DD-1E11-4318-B5AD-2F51DE149660
CaseName          RstHandling.Func3.Case9
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly validate the rcvd  \
                   RST segment while in TIME_WAIT state. RST is valid if       \
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC3_CASE9_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC3_CASE9_

BeginLogPacket RstHandling.Func3.Case9 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { ${TCB.received} != 1 || ${TCB.r_f_ack} != 1} {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Close Conn Simu: EUT doesn't send out ACK."
...
if { $R_Connection_State != $Tcp4StateTimeWait || $R_Status != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Tcp4 - GetModeData."                                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState - $R_Connection_State, ExpectState - $Tcp4StateTimeWait"
...
RecordAssertion pass $GenericAssertionGuid                                     \
                "<TIME_WAIT>: On recv invalid RST, remain in TIME_WAIT"

set TCB.r_acked_seq $L_OrigSeq
# OS --> EUT: ACK
# Expect: EUT --> OS: ACK
set L_TcpFlag $ACK
UpdateTcpSendBuffer TCB -c $L_TcpFlag -a [expr ${TCB.r_isn}+${TCB.r_acked_seq}+${TCB.l_win}+1000000]
SendTcpPacket TCB

set TCB.l_next_seq $L_next_Seq

RecordAssertion pass $Tcp4RstHandlingFunc3AssertionGuid009                     \
                "<TIME_WAIT>: On recv invalid RST, remain in TIME_WAIT"        \
                "Respond to un-acceptable ACK correctly"
