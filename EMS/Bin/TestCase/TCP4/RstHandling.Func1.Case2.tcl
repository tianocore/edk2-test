...
#
CaseGuid          99BE46A1-DD1B-4e84-8BB2-1BB75DF67EBE
CaseName          RstHandling.Func1.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly send out the reset \
                   segment while in ESTABLISHED state.}
################################################################################
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC1_CASE2_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC1_CASE2_

BeginLogPacket RstHandling.Func1.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { $R_Status!=$EFI_SUCCESS || $R_Connection_State!=$Tcp4StateEstablished } {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc1AssertionGuid002                \
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -s [expr ${TCB.l_isn} - 100 ] -p HelloWorld
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "<ESTABLISHED>: OS -> EUT: seq out of windows numbers, "   \
                    "Expect: EUT -> OTS: ACK indicate correct seq"             \
