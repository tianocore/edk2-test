...
#
CaseGuid          7A7A94F3-38DF-47c0-8D43-390264DDC05D
CaseName          WinFlowCtrl.Func4.Case4
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the        \
                   partner's shrinking window when the data retransmission     \
                   happens.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func4.Case5 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
CreatePayload L_Payload incr 1024 0x01
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc4AssertionGuid004                  \
...
#
# When capturing the retransmitted A segment, configure the [OS] to send back
# ACK segments and capture the responded segments separately. The ACK is to
# acknowledge the A segment and change the advertised window to be 1536 octets.
#
ReceiveTcpPacket TCB 5
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the restransmitted segment."
      ...
      if { ${TCB.r_seq} != 1 || ${TCB.r_ack} != 1} {
        ...
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number or acknowledge number of the      \
                         restransmitted segment is not correct."
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the restransmitted segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
