...
#
CaseGuid          6C7ABD6A-0DB8-4b80-B904-1EE5CC919408
CaseName          Routes.Func2.Case1
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality -- Test the          \
functionality of deleting a route.}
################################################################################
...
BeginLogPacket Routes.Func2.Case1 "host $DEF_EUT_IP_ADDR and host              \
                                             $DEF_ENTS_IP_ADDR"
#
...
EFI_TCP4_IO_TOKEN                R_Transmit_IOToken_2
EFI_TCP4_COMPLETION_TOKEN        R_Transmit_CompletionToken
EFI_TCP4_COMPLETION_TOKEN        R_Transmit_CompletionToken_2
...
CHAR8                            R_FragmentBuffer_2($L_FragmentLength_2)
...
SetVar R_FragmentBuffer_2        "8421ABCD"
...
#
# Check Point: Call Tcp4.Routes(),
#              to add a route entry in route table.
#
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Tcp4.Routes() - add/delete a route entry."                    \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
#
ems_output_error [ ReceiveTcpPacket TCB 5]
...
#
# Check Point: Call Tcp4.Routes(),
#              to delete a route entry in route table.
#
SetVar          R_Routes_DeleteRoute       TRUE

Tcp4->Routes {@R_Routes_DeleteRoute, &@R_Routes_SubnetAddress,                 \
              &@R_Routes_SubnetMask, &@R_Routes_GatewayAddress, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4RoutesFunc2AssertionGuid001                       \
                "Tcp4.Routes() - Call Routes() to delete an route entry in     \
                route table."                                                  \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# Call Tcp4.Transmit() to transmit a different packet.
#
BS->CreateEvent "$EVT_NOTIFY_SIGNAL, $EFI_TPL_CALLBACK, 1, &@R_Context,        \
                 &@R_Transmit_CompletionToken_2.Event, &@R_Status"
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid  "BS.CreateEvent."               \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

SetVar R_TxData.DataLength                $L_FragmentLength_2
SetVar R_TxData.FragmentCount             1

SetVar R_FragmentTable.FragmentLength     $L_FragmentLength_2
SetVar R_FragmentTable.FragmentBuffer     &@R_FragmentBuffer_2
SetVar R_TxData.FragmentTable(0)          @R_FragmentTable

SetVar R_Packet_Buffer.TxData  &@R_TxData

SetVar R_Transmit_IOToken_2.CompletionToken         @R_Transmit_CompletionToken_2
SetVar R_Transmit_IOToken_2.Packet_Buffer           @R_Packet_Buffer
SetVar R_Transmit_IOToken_2.CompletionToken.Status  $EFI_INCOMPATIBLE_VERSION

Tcp4->Transmit {&@R_Transmit_IOToken_2, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Tcp4.Transmit - Transmit a packet."                           \
...
while {$timeout > 0} {
  incr timeout -1
  Stall 1
  GetVar R_Transmit_IOToken_2.CompletionToken.Status

  if { ${R_Transmit_IOToken_2.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken_2.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      puts "Data transmission failed."
      RecordAssertion $assert $GenericAssertionGuid "Data transmission failed."\
      "ReturnStatus - ${R_Transmit_IOToken_2.CompletionToken.Status}, ExpectedStatus - $EFI_SUCCESS"
      BS->CloseEvent "@R_Connect_CompletionToken.Event, &@R_Status"
      GetAck
      BS->CloseEvent "@R_Transmit_CompletionToken.Event, &@R_Status"
      GetAck
      BS->CloseEvent "@R_Transmit_CompletionToken_2.Event, &@R_Status"
      GetAck
      CleanUpEutEnvironment
      return
    } else {
