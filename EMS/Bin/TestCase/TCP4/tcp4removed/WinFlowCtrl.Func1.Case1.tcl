...
#
CaseGuid          4BD0AEC2-F004-4227-AC44-9BF846B946E4
...
BeginLogPacket WinFlowCtrl.Func1.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
CreatePayload BoundaryPayload1 Incr 30 0x01
CreatePayload BoundaryPayload2 Incr 32 0x01
CreatePayload BoundaryPayload3 Incr 34 0x01
...
#
UpdateTcpSendBuffer TCB -c $ACK -p BoundaryPayload1
SendTcpPacket TCB
...
UpdateTcpSendBuffer TCB -c $ACK -p BoundaryPayload2
SendTcpPacket TCB
...
UpdateTcpSendBuffer TCB -c $ACK -p BoundaryPayload3
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                          "The EUT should drop the segment with length larger  \
                          than the advertised window size."
  BS->CloseEvent "@R_Accept_CompletionToken.Event, &@R_Status"
  GetAck
  CleanUpEutEnvironment
  return
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                          "EUT drop the segment with length larger than the    \
