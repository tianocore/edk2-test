...
BeginLogPacket Routes.Func1.Case1 "host $DEF_EUT_IP_ADDR and host              \
                                             $DEF_ENTS_IP_ADDR"
#
...
#              to add an route entry in route table.
#
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4RoutesFunc1AssertionGuid001                       \
                "Tcp4.Routes() - Call Routes() to to add an route entry in     \
                route table."                                                  \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { ${TCB.received} == 1 } {
  ...
  set assert fail
  RecordAssertion $assert "EUT doesn't send out any segment."
  BS->CloseEvent "@R_Connect_CompletionToken.Event, &@R_Status"
  GetAck
  CleanUpEutEnvironment
  return
