...
#
CaseGuid          465B5C98-F3A4-4c95-B984-0F8D54C1C02D
...
BeginLogPacket Cancel.Func1.Case1      "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
# Abort the asynchronous connect request.
#
...
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $GenericAssertionGuid                                  \
                "Tcp4.Cancel - Cancel an asynchronous connect request."        \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

set TempStatus  $R_Status

#
# Check the Token.Status to verify the connection has been aborted.
#
while {1 > 0} {
  Stall 1
  ...
  if { $TempStatus != $EFI_SUCCESS }   {
      set assert fail
      RecordAssertion $assert $Tcp4CancelFunc1AssertionGuid001                 \
  ...
  if { ${R_Connect_ConnectionToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Connect_ConnectionToken.CompletionToken.Status} != $EFI_ABORTED} {
      set assert fail
      RecordAssertion $assert $Tcp4CancelFunc1AssertionGuid001                 \
      ...
      set assert pass
      RecordAssertion $assert $Tcp4CancelFunc1AssertionGuid001                 \
                      "Tcp4.Cancel - Call Cancel() to abort the connect        \
