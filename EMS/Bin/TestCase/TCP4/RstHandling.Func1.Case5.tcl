...
#
CaseGuid          5D4042BD-3BF7-4c96-AAC5-C75DCAD7FB84
CaseName          RstHandling.Func1.Case5
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly send out the reset \
                   segment while in FIN_WAIT_1 state.}
################################################################################
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC1_CASE5_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC1_CASE5_

BeginLogPacket RstHandling.Func1.Case5 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
# Now enter FIN_WAIT_1 state
Tcp4->GetModeData {&@R_Connection_State, NULL, NULL, NULL, NULL, &@R_Status}
GetAck
GetVar R_Connection_State
if { $R_Status!=$EFI_SUCCESS || $R_Connection_State!=$Tcp4StateFinWait1 } {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc1AssertionGuid005                \
                  "Active Connection, Enter FIN_WAIT_1"                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState-$R_Connection_State,ExpectedState-Tcp4StateFinWait1"
...
RecordMessage DEFAULT "Enter FIN_WAIT_1 state"
#
# Verify Whether EUT correctly send out RST in FIN_WAIT_1 state
#
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "<FIN_WAIT_1>: OS -> EUT: Unaccept ACK, "                  \
