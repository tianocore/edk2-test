...
#
CaseGuid          8CB5F92A-DBFC-4f0b-8FC0-82F5D3155DFD
CaseName          NagleSWSA.Func3.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the bulk   \
                   ...
                   In a stream of single-byte segments there SHOULD be an ACK  \
...
BeginLogPacket NagleSWSA.Func4.Case3   "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
# Configure the [OS] to send 20 single-byte data segments. There should be at
...
if { $ack_counter < 10 || $ack_counter > 20 } {
  set assert fail
  RecordAssertion $assert $Tcp4NagleSWSAFunc3AssertionGuid003                  \
