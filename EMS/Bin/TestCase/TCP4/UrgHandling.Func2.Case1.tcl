...
#
CaseGuid          8132DF8D-C151-4a6d-9186-C2059322A947
CaseName          UrgHandling.Func2.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly receives urgent    \
                   ...
                   some urgent data between normal data transmission.}
################################################################################
...
BeginLogPacket UrgHandling.Func2.Case1    "host $DEF_EUT_IP_ADDR and host      \
                                                $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -p BoundaryPayload -u 536
SendTcpPacket TCB
ReceiveTcpPacket TCB 5
...
puts "R_RxData.DataLength = ${R_RxData.DataLength}\n"
puts "R_RxData.UrgentFlag = ${R_RxData.UrgentFlag}\n"
...
#
# Receive the first section of urgent data.
#
BS->CloseEvent "@R_Receive_CompletionToken.Event, &@R_Status"
GetAck
BS->CreateEvent "$EVT_NOTIFY_SIGNAL, $EFI_TPL_CALLBACK, 1, &@R_Context,        \
...
#
# Send the remained urgent data and normal data.
#
set L_TcpFlag [expr $URG | $ACK]
UpdateTcpSendBuffer TCB -c $L_TcpFlag -p BoundaryPayload -u 24
SendTcpPacket TCB
ReceiveTcpPacket TCB 5
...
if { ${R_RxData.DataLength} != 25 || ${R_RxData.UrgentFlag} != true } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Receive data length or UrgentFlag is not correct."          \
                  "return DataLength is ${R_RxData.DataLength}, Expected is 25.\
                   return UrgentFlag is ${R_RxData.UrgentFlag}, Expected is true"
...
#
# Receive the remained normal data.
#
BS->CloseEvent "@R_Receive_CompletionToken.Event, &@R_Status"
GetAck
BS->CreateEvent "$EVT_NOTIFY_SIGNAL, $EFI_TPL_CALLBACK, 1, &@R_Context,        \
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4UrgHandlingFunc2AssertionGuid001                  \
                "Tcp4.Receive - Receive data."                                 \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"
...
if { ${R_RxData.DataLength} != 999 || ${R_RxData.UrgentFlag} != false } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Receive data length or UrgentFlag is not correct."          \
                  "return DataLength is ${R_RxData.DataLength}, Expected is 1000.\
                   return UrgentFlag is ${R_RxData.UrgentFlag}, Expected is false"
