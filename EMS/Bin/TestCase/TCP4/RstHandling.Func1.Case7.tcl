...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC1_CASE7_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC1_CASE7_

BeginLogPacket RstHandling.Func1.Case7 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
# Now enter CLOSING state
#
Tcp4->GetModeData {&@R_Connection_State, NULL, NULL, NULL, NULL, &@R_Status}
GetAck
GetVar R_Connection_State
if { $R_Status!=$EFI_SUCCESS || $R_Connection_State!=$Tcp4StateClosing } {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc1AssertionGuid007                \
                  "Active Connection, Enter CLOSING"                           \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState-$R_Connection_State,ExpectedState-$Tcp4StateClosing"
...
RecordMessage DEFAULT "Enter CLOSING state"

#
# Verify Whether EUT correctly send out RST in CLOSING state
#
#
...
set L_TcpFlag $ACK
UpdateTcpSendBuffer TCB -c $L_TcpFlag -a [expr $L_CorrectAck+${TCB.l_win}+100]
SendTcpPacket TCB
