...
#
CaseGuid          12CAB8C4-E0FB-40b8-A627-80057553832A
CaseName          Options.Func3.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handle the         \
                   ...
                   boundary, one item of the same option is splitted in        \
                   different words.}
################################################################################
...
BeginLogPacket Options.Func5.Case3 "host $DEF_EUT_IP_ADDR and host             \
                                         $DEF_ENTS_IP_ADDR"

#
# Build combined options field as the following:
# segment with the No-Operation option between multiple options at the word
# boundary, one item of the same option is splitted in different words.
#
CreatePayload CombinedOptions Data 8 0x01 0x02 0x04 0x01 0x00 0x01 0x01 0x00
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $Tcp4OptionsFunc3AssertionGuid003                  \
