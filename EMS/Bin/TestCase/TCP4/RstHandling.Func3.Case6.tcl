...
#
CaseGuid          5953C490-11FD-4453-A6FA-9FA58ABFD18B
CaseName          RstHandling.Func3.Case6
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly validate the rcvd  \
                   RST segment while in FIN_WAIT_2 state. RST is valid if      \
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC3_CASE6_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC3_CASE6_

BeginLogPacket RstHandling.Func3.Case6 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { $R_Connection_State != $Tcp4StateFinWait2 || $R_Status != $EFI_SUCCESS} {
  ...
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Tcp4 - GetModeData."                                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState - $R_Connection_State, ExpectState - $Tcp4StateFinWait2"
...
if { $R_Connection_State != $Tcp4StateFinWait2 || $R_Status != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc3AssertionGuid006                \
                  "Tcp4 - GetModeData."                                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState - $R_Connection_State, ExpectState - $Tcp4StateFinWait1"
...
set assert pass
RecordAssertion $assert $GenericAssertionGuid                                  \
                "<FIN_WAIT_2>: On recv invalid RST, remain in FIN_WAIT_2"
