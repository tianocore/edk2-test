...
#
CaseGuid          9FC68ABF-E1B6-4de8-A73B-90FEB6DF2F22
CaseName          Options.Func6.Case2
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This item is to test [EUT] could disable timestamp option\
                   when it receives <SYN, ACK> segment without timestamp option\
                   -- The echo field of timestamp option of [EUT] <SYN> segment\
                   should be zero.                                             \
                   -- Both the echo field and value field of timestamp option  \
                   of following other segments should be zero.}
################################################################################
...
BeginLogPacket Options.Func8.Case2 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
#
set    L_FragmentLength          2528   ;# data to transmit by [EUT]
...
SetVar R_Configure_ControlOption.EnableTimeStamp      TRUE
  ...
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 } {
      ...
      return
        ...
        CleanUpEutEnvironmentEnd
        return
    } elseif { $index == 1 && ${TCB.r_ts_ecr} != 0 } {
      puts "{TCB.r_ts_ecr} = ${TCB.r_ts_ecr}"
      RecordAssertion fail $GenericAssertionGuid                               \
                      "The echo field in timestamp option of <SYN> segment should be found zero."
      ...
      CleanUpEutEnvironmentEnd
      return
    } elseif { $index > 1 && ${TCB.r_ts_val} != 0 && ${TCB.r_ts_ecr} != 0 } {
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      ...
      RecordAssertion $assert $Tcp4OptionsFunc6AssertionGuid002                \
