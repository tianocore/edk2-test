...
#
CaseGuid          BF3AA48D-6075-479a-A35F-1B3D0E3A5F08
CaseName          CnntOpening.Func3.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly refuses the        \
                   attempted connections from broadcast and multicast address. \
                   (In fact, IP Layer has already discarded this packet, no    \
                   such packet sent up to TCP)}
################################################################################
...
BeginLogPacket CnntOpening.Func4.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntOpeningFunc3AssertionGuid001                  \
...
if { ${TCB.received} == 1 } {
  set assert fail
  puts "EUT should not send any segment to respond the request from boradcast  \
        or multicast address"
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT should not send any segment to respond the      \
                   request from boradcast or multicast address."
