...
#
CaseGuid          E744B75D-E8E0-462d-B1A8-8046EB622F00
CaseName          RstHandling.Func1.Case6
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly send out the reset \
                   segment while in FIN_WAIT_2 state.}
################################################################################
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC1_CASE6_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC1_CASE6_

BeginLogPacket RstHandling.Func1.Case6 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
# Now enter FIN_WAIT_2 state
#
Tcp4->GetModeData {&@R_Connection_State, NULL, NULL, NULL, NULL, &@R_Status}
GetAck
GetVar R_Connection_State
if { $R_Status!=$EFI_SUCCESS || $R_Connection_State!=$Tcp4StateFinWait2 } {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc1AssertionGuid006                \
                  "Active Connection, Enter FIN_WAIT_2"                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState-$R_Connection_State,ExpectedState-$Tcp4StateFinWait2"
...
RecordMessage DEFAULT "Enter FIN_WAIT_2 state"

#
# Verify Whether EUT correctly send out RST in FIN_WAIT_2 state
#
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "<FIN_WAIT_2>: OS -> EUT: Unaccept ACK, "                  \
