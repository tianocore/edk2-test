...
#
CaseGuid          A040AE9D-1EC8-434a-B51E-81544A9CDDF4
CaseName          Options.Func2.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handle the         \
                   End-of-Options option, there are more options behind the    \
                   End-of-Options option. These options should be ignored.}
################################################################################
...
BeginLogPacket Options.Func4.Case2 "host $DEF_EUT_IP_ADDR and host             \
                                         $DEF_ENTS_IP_ADDR"

#
# Build an option field as the following.
# No-Option No-Option No-Option End-of-Options Option
# MSS10-Option (this option should be ignored)
#
CreatePayload CombinedOptions Data 8 0x01 0x01 0x01 0x00 0x02 0x04 0x01 0x00
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $Tcp4OptionsFunc2AssertionGuid002                  \
