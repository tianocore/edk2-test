...
#
CaseGuid          FF5EFC01-BE69-4c19-A5D8-08C999E9BDE5
CaseName          CnntClosing.Func1.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the        \
                   simultaneous closing connection.}
################################################################################
...
BeginLogPacket CnntClosing.Func1.Case3 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntClosingFunc1AssertionGuid003                  \
...
set L_AckNum [expr ${TCB.r_isn} + 1]
UpdateTcpSendBuffer TCB -c $L_TcpFlag -a $L_AckNum
SendTcpPacket TCB
  ...
  if { ${TCB.r_f_ack} != 1 } {
    ...
    return
      ...
      if { ${TCB.r_ack} != 2} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The acknowledge number of FIN/ACK segment is not correct."
        CleanUpEutEnvironment
        return
      }
    }
...
set L_AckNum [expr ${TCB.r_isn} + 2]
UpdateTcpSendBuffer TCB -c $ACK -a $L_AckNum
SendTcpPacket TCB
