...
#
CaseGuid          61142542-AD34-4b6a-B5F2-15950DCD3801
CaseName          WinFlowCtrl.Func5.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly probes a partner's \
                   advertised zero window.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func5.Case3 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc5AssertionGuid002                  \
...
for { set i 1} { $i < 9 } { incr i } {
  ...
  if { ${TCB.received} == 1 } {
    ...
    set assert pass
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "The EUT send out the window probing segment correctly."
    UpdateTcpSendBuffer TCB -c $ACK -w 0
    SendTcpPacket TCB
  } else {
    set assert fail
