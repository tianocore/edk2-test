...
#
CaseGuid          70B40B8B-6A47-49ca-B60A-859F0D856FCC
CaseName          WinFlowCtrl.Func5.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly probes a partner's \
                   advertised zero window, when partner advertises non-zero    \
                   window, EUT can send out left data segments correctly.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func5.Case6 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
#
set    L_FragmentLength          1069   ;# default_mss * 2 - 3
...
CreatePayload DataPayload incr 536 0x01
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -w 536
SendTcpPacket TCB
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc5AssertionGuid003                  \
    ...
    return
      ...
      if { ${TCB.r_seq} != 1 || ${TCB.r_ack} != 1} {
        ...
        RecordAssertion $assert $GenericAssertionGuid                          \
                        ...
                        the first data segment is not correct."
...
#
# Make the [OS] send an acknowledge segment with a zero window.
# In current implementation, the Zero window probing segment contains no data.
#
UpdateTcpSendBuffer TCB -c $ACK -w 0
SendTcpPacket TCB

set remote_seq 536
set temp_ack [expr ${TCB.r_isn} + 537]
for { set i 0} { $i < 5 } { incr i } {
  ReceiveTcpPacket TCB 64
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 } {
      set assert fail
      RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT doesn't send out the Zero window Probing segment correctly."
      ...
      return
        ...
        if { ${TCB.r_seq} != $remote_seq || ${TCB.r_ack} != 1} {
          set assert fail
          RecordAssertion $assert $GenericAssertionGuid                        \
                          "The sequence number or acknowledge number of the    \
                           window probing segment is not correct."
        ...
        if { ${TCB.r_len} != 0 } {
          set assert fail
          RecordAssertion $assert $GenericAssertionGuid                        \
                          "The length of the Zero window Probing segment is incorrect."
    ...
    set assert pass
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "The EUT send out the ACK segment correctly."
    UpdateTcpSendBuffer TCB -c $ACK -w 0 -a $temp_ack
    SendTcpPacket TCB
  } else {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
...
#
# Expand the window to the size just equal to the left data size. [EUT] should
# send out all the left data together.
#
set L_left_data_length [expr $L_FragmentLength - $remote_seq]

UpdateTcpSendBuffer TCB -c $ACK -w $L_left_data_length -a $temp_ack
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the last data segment correctly."
    ...
    return
      ...
      set remote_seq [expr $remote_seq + 1]
      if { ${TCB.r_seq} != $remote_seq || ${TCB.r_ack} != 1} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
      ...
      if { ${TCB.r_len} != $L_left_data_length } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the last data segment is not correct"
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the last data segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
