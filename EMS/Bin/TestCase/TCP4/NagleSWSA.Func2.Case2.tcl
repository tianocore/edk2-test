...
#
CaseGuid          EFB6C8BE-3031-4571-BE8C-F89EF4D223D8
CaseName          NagleSWSA.Func2.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the small  \
                   segments in accord with Nagle algorithm, when retranmission \
                   happens, the accumulated small segments should be sent out  \
                   together.}
################################################################################
...
BeginLogPacket NagleSWSA.Func2.Case2   "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4NagleSWSAFunc2AssertionGuid002                    \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# OS get the first transmitted data segment.
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't retranmist the data segment correctly."
    CleanUpEutEnvironment
    return
  } else {
      if { ${TCB.r_seq} != 1 || ${TCB.r_ack} != 1} {
        ...
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number or acknowledge number of the      \
                         retransmission segment is not correct."
        CleanUpEutEnvironment
        return
      }
      if { ${TCB.r_len} != 1 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the retransmission segment is incorrect."
        CleanUpEutEnvironment
        return
      }
    }
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT retranmist the data segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
