...
#
CaseGuid          5AA3F8F4-66B3-410d-BEE8-4ABCB2A66541
CaseName          WinFlowCtrl.Func5.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly receives data      \
                   segments while its partner's advertised window is zero.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func5.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc5AssertionGuid001                  \
...
#
set remote_should_ack 1
for { set i 0} { $i < 4 } { incr i } {
  UpdateTcpSendBuffer TCB -c $ACK -w 0 -p DataPayload
  SendTcpPacket TCB
  ReceiveTcpPacket TCB 5
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 } {
      set assert fail
      RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT doesn't send out the ACK segment correctly."
      ...
      return
        ...
        set remote_should_ack [expr $remote_should_ack + 536]
        if { ${TCB.r_seq} != 537 || ${TCB.r_ack} != $remote_should_ack} {
          set assert fail
          RecordAssertion $assert $GenericAssertionGuid                        \
                          "The sequence number or acknowledge number of the first\
                           data segment is not correct."
        ...
        if { ${TCB.r_len} != 0 } {
         	set assert fail
          RecordAssertion $assert $GenericAssertionGuid                        \
                          "The length of the ACK segment is incorrect"
