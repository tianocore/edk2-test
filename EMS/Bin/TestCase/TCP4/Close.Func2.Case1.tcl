...
#
CaseGuid          00B898F6-5CBD-4e62-B2F2-A24E75EC27FA
CaseName          Close.Func2.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the TCP.Close interface. Abort the TCP \
                   connection on close instead of the standard TCP close       \
                   process by setting the AbortOnClose to TRUE.}
################################################################################
...
BeginLogPacket Close.Func2.Case1       "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CloseFunc2AssertionGuid001                        \
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_rst} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the RST segment correctly."
    ...
    return
      ...
      if { ${TCB.r_seq} != 1 } {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number of the RST data segment is incorrect."
        CleanUpEutEnvironment
        return
      }
    }
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the RST segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
