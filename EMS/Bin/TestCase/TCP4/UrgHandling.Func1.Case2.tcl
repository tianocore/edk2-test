...
#
CaseGuid          E106FD8B-5237-430b-BFD1-07F92A2B6259
CaseName          UrgHandling.Func1.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly uses the urgent    \
                   pointer to denote the last urgent octet of urgent data. The \
                   urgent data exceed the maximum number of urgent pointer.}
################################################################################
...
BeginLogPacket UrgHandling.Func1.Case2    "host $DEF_EUT_IP_ADDR and host      \
                                                $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4UrgHandlingFunc1AssertionGuid002                  \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# Get the tranmistted data segment and check the urgent pointer, it should point
# to the sequence number of the LAST octet
#
ReceiveTcpPacket TCB 5

if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_urg} != 1} {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the URG data segment correctly."
    ...
    return
      ...
      if { ${TCB.r_len} != 512 || ${TCB.r_urg} != 65535 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length or urgent pointer of the data segment \
                         is not correct."
...
#
# The urgent pointer will roolback but the EUT should maintain the correct value
# of the urgent pointer. After sending out the first data segment, EUT should
# send the second data segment with urgent pointer = 65536 - 512 = 65024.
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_urg} != 1} {
    ...
    return
      ...
      if { ${TCB.r_len} != 512 || ${TCB.r_urg} != 65023 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length or urgent pointer of the data CleanUpEutEnvironmentEnd"
        return
      }
    }
  set assert pass
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_CONNECTION_RESET} {
      set assert fail
      RecordAssertion $assert $GenericAssertionGuid                            \
