...
#
CaseGuid          823D2D9E-F4EF-4298-8720-3E69A98C54B1
CaseName          RstHandling.Func2.Case6
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly handles the        \
                   reception of a RST segment in FIN_WAIT_2 state              \
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC2_CASE7_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC2_CASE7_

BeginLogPacket RstHandling.Func2.Case7 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
# Currently, enter ESTABLISHED state
...
if { $R_Connection_State!=$Tcp4StateFinWait2 || $R_Status!=$EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Active Connection, Enter ESTABLISHED - 1"                   \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState - $R_Connection_State, ExpectedState - FIN_WAIT_2"
...
RecordMessage DEFAULT "Passive Connection: Enter FIN_WAIT_2 state"
...
if { $R_Connection_State != $Tcp4StateClosed || $R_Status != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $Tcp4RstHandlingFunc2AssertionGuid006                \
                  "Active Connection, In <FIN_WAIT_2> state:"                  \
...
RecordAssertion pass $GenericAssertionGuid                                     \
                "Active Connection, In <FIN_WAIT_2> state: "                   \
                "On recv RST (seqid EQUAL TO expect value), Return CLOSED state"
