...
#
CaseGuid          25C3D84F-0A23-54a7-A584-6C5BC4ED2F63
CaseName          Others.Func4.Case2
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- In LAST_ACK state, configure OS to send FIN to EUT.      \
...
BeginLogPacket Others.Func4.Case2 "host $DEF_EUT_IP_ADDR and host              \
                                             $DEF_ENTS_IP_ADDR"
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_fin} != 1 || ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT failed to send out FIN|ACK back in CLOSE_WAIT state."
...
#
# Calling Tcp4.GetModeData(), Now EUT in LAST_ACK state.
#
Tcp4->GetModeData {&@R_Tcp4State, &@R_Configure_Tcp4ConfigData, &@R_Ip4ModeData,
                   &@R_MnpConfigData, &@R_SnpModeData, &@R_Status}
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4OthersFunc4AssertionGuid002                       \
                "Tcp4.GetModeData - Calling GetModeData."                      \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

GetVar R_Tcp4State
if { $R_Tcp4State != $Tcp4StateLastAck } {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT failed to enter into LAST_ACK state."
...
set assert pass
RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT succeeded to enter into LAST_ACK state."

#
# OS sends <FIN> segment (This FIN segment is not expected)
#
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT failed to send out ACK back to problematic FIN in LAST_ACK state."
...
if { $L_FirstAckSeq == $L_SecondAckSeq } {
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT doesn't ACK the problematic FIN when in LAST_ACK state."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                            \
                      "EUT mistakenly ACK the problematic FIN when in LAST_ACK state."
}
