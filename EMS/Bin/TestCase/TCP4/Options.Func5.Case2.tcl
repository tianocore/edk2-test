...
#
CaseGuid          3E5C31DE-D289-4e06-972E-B4F8D8C8835A
CaseName          Options.Func5.Case2
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This item is to test the [EUT] correctly ignores a Window\
                   scale option in a segment without SYN bit set.}
################################################################################
...
BeginLogPacket Options.Func7.Case2 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
#
set    L_FragmentLength          1000    ;# data to transmit by [EUT]
...
#
CreatePayload Option_MSS_WS Data 8 0x02 0x04 $L_MSSH $L_MSSL 0x03 0x03 $L_WindowScale 0x01

#
# Create TCP option.
# Another Windows Scale Value which will be sent in <ACK> Segment.
#
set L_WindowScale_new [ expr $L_WindowScale + 1 ]  ;# increase or decrease is OK
CreatePayload Option_WS Data 4 0x03 0x03 $L_WindowScale_new 0x01
...
while { $sum_fragment < $L_FragmentLength } {
  ...
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 } {
      ...
      return
        ...
        CleanUpEutEnvironmentEnd
        return
    } elseif { ${TCB.r_len} > $L_MSS } {              ;# check r_len > MSS?
        puts "{TCB.r_len} = ${TCB.r_len}"
  ...
  #
  # [OS] send <ACK> segment, with another window scale option.
  #
  UpdateTcpSendBuffer TCB -c $ACK -w $L_WindowField -o Option_WS
  SendTcpPacket TCB
}
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc5AssertionGuid002                \
