...
#
CaseGuid          1318E84C-BA90-48a7-8582-2A52C4E6AC29
CaseName          Transmit.Func1.Case3
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                  -- Check the validity of [PSH] bit during data transimission,\
                  by sending 1024 bytes data segment to [EUT],                 \
                  with [ENTS] default MSS = 536.}
################################################################################
...
BeginLogPacket Transmit.Func1.Case3 "host $DEF_EUT_IP_ADDR and host            \
                                             $DEF_ENTS_IP_ADDR"
#
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4TransmitFunc1AssertionGuid003                     \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# ENTS get the transmitted data packet.
#
ReceiveTcpPacket TCB 5

#
# Check Point: [PSH] bit and [ACK] bit should both be set in last TCP segment,
#              while previous TCP segment should not be set.
#              according to following two conditions:
#              (1) EFI_TCP4_TRANSMIT_DATA.Push set to TRUE and,
#              (2) Two segment data (1024 = 536 + 488) are to be transmitted.
#
# receive the first TCP segment
#
if { ${TCB.received} == 1 } {
  #
  # [ENTS] send ACK segment data.
  #
  UpdateTcpSendBuffer TCB -c $ACK
  SendTcpPacket TCB

  #
  # check PSH and ACK bit
  #
  if { ${TCB.r_f_psh} != 0 || ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out <ACK> segment correctly."
#    CleanUpEutEnvironmentEnd
...
#
# receive the second TCP segment data.
#
ReceiveTcpPacket TCB 5

if { ${TCB.received} == 1 } {
  #
  # ENTS send ACK segment
  #
  UpdateTcpSendBuffer TCB -c $ACK
  SendTcpPacket TCB
  ...
  # check PSH and ACK bit
  #
  if { ${TCB.r_f_psh} != 1 || ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out <PSH, ACK> segment correctly."
