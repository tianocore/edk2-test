...
#
CaseGuid          CE8E1AEA-D15E-46df-9EF5-C576FE0E0366
CaseName          WinFlowCtrl.Func1.Case3
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly receives the       \
                   segment that has the advertised receive window open         \
                   right-edge and include the duplicated ACKs.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func1.Case4 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc1AssertionGuid003                  \
...
#
# window update segment $GenericAssertionGuid                                  \
#                        including duplicated ACKs should be discarded.
#
set temp_ack [expr ${TCB.r_isn} + 1]
UpdateTcpSendBuffer TCB -c $ACK -w 4096 -a $temp_ack
SendTcpPacket TCB
