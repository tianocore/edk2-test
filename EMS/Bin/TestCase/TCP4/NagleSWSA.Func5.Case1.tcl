...
#
CaseGuid          163E8F94-18BD-466a-8D5D-139E712E8EEE
CaseName          NagleSWSA.Func5.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly avoids the Silly   \
                   ...
                   transmit unless everything can be sent out and no need to   \
                   wait ACK.}
################################################################################
...
BeginLogPacket NagleSWSA.Func6.Case1   "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o MssOption -w 1034
SendTcpPacket TCB
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4NagleSWSAFunc5AssertionGuid001                    \
...
#
# EUT should send out all the left data segments.
#
ReceiveTcpPacket TCB 5
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    ...
    return
      ...
      if { ${TCB.r_len} != 30 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the left data segment is Environment"
        return
      }
    }
  set assert pass
