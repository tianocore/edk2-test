...
#
CaseGuid          03C9C056-447D-4f2a-943D-3FB20D6BB2BB
CaseName          UrgHandling.Func1.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly uses the urgent    \
                   pointer to denote the last urgent octet of urgent data.}
################################################################################
...
BeginLogPacket UrgHandling.Func1.Case1    "host $DEF_EUT_IP_ADDR and host      \
                                                $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4UrgHandlingFunc1AssertionGuid001                  \
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_urg} != 1} {
    ...
    return
      ...
      if { ${TCB.r_len} != 512 || ${TCB.r_urg} != 5119 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
