...
#
CaseGuid          1B187AE2-B64B-4e31-9C17-FC9A7B46B1EF
CaseName          Options.Func2.Case1
CaseCategory      TCP
...
BeginLogPacket Options.Func4.Case1 "host $DEF_EUT_IP_ADDR and host             \
                                         $DEF_ENTS_IP_ADDR"
CreatePayload HelloWorld STRING 11 HelloWorld

#
# Build an option field containing End-of-Options Option.
#
CreatePayload CombinedOptions Data 4 0x01 0x01 0x01 0x00
...
#
# Configure the [OS] to send tcp segment with CombinedOptions.
#
#
UpdateTcpSendBuffer TCB -c $ACK -o CombinedOptions -p HelloWorld
SendTcpPacket TCB
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert $Tcp4OptionsFunc2AssertionGuid001                  \
