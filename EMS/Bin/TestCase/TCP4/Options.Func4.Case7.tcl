...
#
CaseGuid          276EFCC3-9768-4ae4-84FD-660659307CA4
CaseName          Options.Func4.Case7
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   ...
                   --Let MSS > 1460. [EUT] should ignore MSS larger than 1460  \
                   and replace it with 1460.}
################################################################################
...
BeginLogPacket Options.Func6.Case7 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
#
...
#
set    L_FragmentLength          409600  ;# data size to be transmitted by [EUT]
set    RFC_TCP_MAX_MSS           1460    ;# RFC MAX MSS, EFI conform with it.
set    L_MSS                     $RFC_TCP_MAX_MSS    ;# [EUT] will use 1460 as
                                                     ;# [OS].mss when [OS]
                                                     ;# send MSS > 1460
...
#
# Build TCP MSS option, MSS = 2048, invalid value, larger than 1460(Maximum MSS)
#
CreatePayload OptionMSS_OVERFLOW Data 4 0x02 0x04 0x08 0x00
...
UpdateTcpSendBuffer TCB -c $L_TcpFlag -o OptionMSS_OVERFLOW
SendTcpPacket TCB
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc4AssertionGuid007                \
