...
#
CaseGuid          25A3E84F-AA90-64a7-2584-BE5BC4E6CC10
CaseName          Others.Func3.Case1
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                  -- Configure OS to send junky data after <FIN,ACK> segment,  \
                  EUT should reset the connection.}
################################################################################
...
BeginLogPacket Others.Func3.Case1 "host $DEF_EUT_IP_ADDR and host              \
                                             $DEF_ENTS_IP_ADDR"
#
...
CreatePayload Payload_1 incr $L_FragmentLength  0x01
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_rst} != 1 } {
    set assert fail
    RecordAssertion $assert $Tcp4OthersFunc3AssertionGuid001                   \
