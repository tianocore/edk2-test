...
#
CaseGuid          A0152B6E-6172-41ba-AD60-90D42AD9CABD
CaseName          WinFlowCtrl.Func4.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles a link     \
                   ...
                   case test Right Edge Shrinks with Left Edge Keeps.}
################################################################################
...
BeginLogPacket WinFlowCtrl.Func4.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4WinFlowCtrlFunc4AssertionGuid002                  \
...
UpdateTcpSendBuffer TCB -c $ACK -w 2048 -a $temp_ack
SendTcpPacket TCB
  ...
  if { ${TCB.r_f_ack} != 1 } {
    ...
    return
      ...
      if { ${TCB.r_seq} != 1 || ${TCB.r_ack} != 1} {
        set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "EUT should retranmist the first data segment."
