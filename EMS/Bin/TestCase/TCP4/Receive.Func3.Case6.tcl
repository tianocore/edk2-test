...
#
CaseGuid          25C5D83D-FA90-27a8-2284-BF3B29E6CC85
CaseName          Receive.Func3.Case6
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                  ...
                  |              |     |             |      |              |   \
                  |_________ ____|_____|_____________|______|______________|   \
                  <---- Segment 1 ---->                                        \
                                                     <----- Segment 2 ---->    \
                                  <------ Segment 3 ------->                   \
                  .}
...
BeginLogPacket Receive.Func3.Case6 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
#
...
set    L_FragmentLength_1        300
set    L_FragmentLength_2        400
set    L_FragmentLength_3        500
set    L_OverlapLength_13        100
set    L_OverlapLength_23        200
set    L_SpaceLength_12          [ expr $L_FragmentLength_3 \
                                      - $L_OverlapLength_13 \
                                      - $L_OverlapLength_23 ]

set    EUT_RECEIVED_DATA_SEQ     [ expr $L_FragmentLength_1 \
                                      + $L_FragmentLength_2 \
                                      + $L_FragmentLength_3 \
                                      - $L_OverlapLength_13 \
                                      - $L_OverlapLength_23 ]

if { $L_SpaceLength_12 <= 0 } {
  puts "Space between Segment 1 & Segment 2 no more than 0, \
                    overlapped, please make sure larger than zero."
  return
}
...
EFI_TCP4_FRAGMENT_DATA           R_FragmentTable_3
CHAR8                            R_FragmentBuffer_1($L_BufferLength)
CHAR8                            R_FragmentBuffer_2($L_BufferLength)
CHAR8                            R_FragmentBuffer_3($L_BufferLength)

BuildTcpOptions MssOption -m 1460

CreatePayload Payload_1 Const $L_FragmentLength_1  0x01
CreatePayload Payload_2 Const $L_FragmentLength_2  0x02
CreatePayload Payload_3 Const $L_FragmentLength_3  0x03
...
# OS send DATA & ACK segment
#
set tmp_seq   [ expr ${TCB.l_isn} + ${TCB.l_next_seq} + $L_SpaceLength_12 ]
set L_TcpFlag [ expr $PSH | $ACK ]
...
# OS send DATA & ACK segment
#
set tmp_seq   [ expr ${TCB.l_isn} + $L_FragmentLength_1 - $L_OverlapLength_13 ]
set L_TcpFlag [ expr $PSH | $ACK ]
UpdateTcpSendBuffer TCB -c $L_TcpFlag -p Payload_3 -s $tmp_seq
SendTcpPacket TCB
...
SetVar R_RxData.DataLength                [ expr $L_FragmentLength_1 + $L_FragmentLength_2 + $L_FragmentLength_3 ]
SetVar R_RxData.FragmentCount             3
...
SetVar R_FragmentTable_3.FragmentLength   $L_FragmentLength_3
SetVar R_FragmentTable_3.FragmentBuffer   &@R_FragmentBuffer_3
SetVar R_RxData.FragmentTable(2)          @R_FragmentTable_3
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4ReceiveFunc3AssertionGuid006                      \
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    ...
    if { ${TCB.r_ack} != $EUT_RECEIVED_DATA_SEQ + 1 } {
      puts "{TCB.r_ack} = ${TCB.r_ack}\n"
      puts "received data length should be $EUT_RECEIVED_DATA_SEQ ]\n"
    }
  }
} else {
  set assert fail
