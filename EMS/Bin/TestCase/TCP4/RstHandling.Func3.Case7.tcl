...
#
CaseGuid          F4A88B57-AD73-441a-8DC4-A07911D0C638
CaseName          RstHandling.Func3.Case7
CaseCategory      TCP
CaseDescription   {This item is to test the <EUT> correctly validate the rcvd  \
                   RST segment while in CLOSE_WAIT state. RST is valid if      \
...
proc CleanUpEutEnvironmentEnd {} {
  EndLogPacket
  EndScope _TCP4_RSTHANDLING_FUNC3_CASE7_
  EndLog
}
...
# BeginScope on OS.
#
BeginScope _TCP4_RSTHANDLING_FUNC3_CASE7_

BeginLogPacket RstHandling.Func3.Case7 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { $R_Connection_State != $Tcp4StateCloseWait || $R_Status != $EFI_SUCCESS} {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "Tcp4 - GetModeData."                                        \
                  "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"    \
                  "CurState - $R_Connection_State, ExpectState - $Tcp4StateCloseWait"
...
RecordAssertion pass $GenericAssertionGuid                                     \
                "<CLOSE_WAIT>: On recv invalid RST, remain in CLOSE_WAIT"

set TCB.r_acked_seq $L_OrigSeq
set TCB.l_next_seq  [expr $L_next_Seq - 1]
# OS --> EUT: FIN|ACK
# Expect: EUT --> OS: ACK
...
#  RecordAssertion $assert $GenericAssertionGuid                               \
                   "<CLOSE_WAIT>: On recv invalid RST, remain in CLOSE_WAIT"   \
#                  "can't shutdown normally"
#  CleanUpEutEnvironmentEnd
#  return
#}

RecordAssertion pass $Tcp4RstHandlingFunc3AssertionGuid007                     \
                "<CLOSE_WAIT>: On Received retransmission FIN, remain in CLOSE_WAIT" \
