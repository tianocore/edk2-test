...
#
CaseGuid          267BC2A4-370E-48e7-B98B-01C8FF039761
CaseName          CnntClosing.Func1.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the closing\
                   connection when it initiates the closing.}
################################################################################
...
BeginLogPacket CnntClosing.Func1.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntClosingFunc1AssertionGuid001                  \
      ...
      if { ${TCB.r_seq} != 1 || ${TCB.r_ack} != 1} {
        ...
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The sequence number or acknowledge number of          \
                         FIN/ACK segment is not correct."
        CleanUpEutEnvironment
        return
      }
    }
