...
#
CaseGuid          8DBA120C-598B-4fbc-822C-79A713BE5F0E
CaseName          CnntClosing.Func2.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles a valid    \
                   SYN segment while it is in TIME-WAIT state.}
################################################################################
...
BeginLogPacket CnntClosing.Func2.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntClosingFunc2AssertionGuid002                  \
...
#
# Send a SYN segment with a larger sequence number than the previous connection
# contained.
#
set L_SeqNew [expr ${TCB.l_next_seq} + 100]
UpdateTcpSendBuffer TCB -c $SYN -s $L_SeqNew
SendTcpPacket TCB
ReceiveTcpPacket TCB 5

#
# If the SYN is not in the window, an ack should be sent out (sequence number
# check).
#
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
