...
#
CaseGuid          82EA30FC-992F-291c-B149-3AC26B9388CD
CaseName          Options.Func5.Case8
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This case is designed to test when [OS]'s scaled window  \
                   size larger than [OS]'s MSS, here, (256<<2) > 800, [EUT]    \
                   could correctly send segment data with length small than MSS\
...
BeginLogPacket Options.Func7.Case8 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
#
CreatePayload  Option_MSS_WS  Data  12                            \
                                    0x02 0x04 $L_MSSH $L_MSSL     \
                                    0x01 0x01 0x03 0x03           \
                                    $L_WindowScale 0x01 0x01 0x01
#
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc5AssertionGuid008                \
