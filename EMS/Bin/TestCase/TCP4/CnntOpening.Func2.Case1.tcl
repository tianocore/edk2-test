...
#
CaseGuid          9CD4E36A-9C65-42fe-AE60-5B03EE671FC9
CaseName          CnntOpening.Func2.Case1
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handles the flag   \
                   combination: ACK, FIN through active open.}
################################################################################
...
BeginLogPacket CnntOpening.Func2.Case1 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4CnntOpeningFunc2AssertionGuid001                  \
...
#
# EUT should ignore this unexpected segment and retransmit the SYN segment.
#
ReceiveTcpPacket TCB 5
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_syn} != 1 } {
    set assert fail
    puts "EUT doesn't resend out SYN segment correctly."
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out SYN segment correctly."
    ...
    if { ${TCB.r_seq} != 0 } {
      set assert fail
      puts "The sequence number in EUT's resent SYN segment is not correct."
      RecordAssertion $assert $GenericAssertionGuid                            \
                      "The sequence number in EUT's SYN segment is not correct."
