...
#
CaseGuid          30F7A5E2-ABA9-4250-83E5-268D48BECEF7
CaseName          Options.Func5.Case4
CaseCategory      TCP
CaseDescription   {This case is to test the Functionality.                     \
                   -- This item is to test the [EUT] correctly handles the     \
                   segment with window scaling shift count exceeding 14.}
################################################################################
...
BeginLogPacket Options.Func7.Case4 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
...
set    L_MSS                     1460    ;# MSS claimed by [OS]
set    L_WindowField             64      ;# [OS] TCP header - Window Field
set    L_WindowScale             16      ;# [OS] TCP option - Window Scale
#
# Calculate [OS] MAX acceptable window
#
if { $L_WindowScale >= 0 && $L_WindowScale <= 14 } {
  set    L_RCV_WND               [ expr $L_WindowField << $L_WindowScale ]
} else {
  set    L_RCV_WND               [ expr $L_WindowField << 14 ]
}
  ...
  if { ${TCB.received} == 1 } {
    if { ${TCB.r_f_ack} != 1 } {
      ...
      return
        ...
        CleanUpEutEnvironmentEnd
        return
    } elseif { ${TCB.r_scalwin_shif} > 14  } {       ;# check r_scalwin_shif = 14?
        puts "{TCB.r_scalwin_shif} = ${TCB.r_scalwin_shif}"
        puts "Expected Segment Data Size should be no more than 14."
        RecordAssertion fail $GenericAssertionGuid                             \
                        "EUT send TCP Segment Data Size larger than 14."
...
while {1 > 0} {
  ...
  if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_INCOMPATIBLE_VERSION} {
    if { ${R_Transmit_IOToken.CompletionToken.Status} != $EFI_SUCCESS} {
      set assert fail
      RecordAssertion $assert $Tcp4OptionsFunc5AssertionGuid004                \
