...
#
CaseGuid          E0F56ADF-4C96-46bf-9122-1B15B2AC0B0D
CaseName          UrgHandling.Func3.Case2
CaseCategory      TCP
CaseDescription   {This item is to test the [EUT] correctly handle the urgent  \
                   data transmission when communication peer's receive window  \
                   is zero.}
################################################################################
...
BeginLogPacket UrgHandling.Func3.Case2 "host $DEF_EUT_IP_ADDR and host         \
                                             $DEF_ENTS_IP_ADDR"
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    set assert fail
    RecordAssertion $assert "EUT doesn't send out the first data segment correctly."
    ...
    return
      ...
      if { ${TCB.r_len} != 536 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the first data segment is Environment"
        return
      }
    }
  set assert pass
...
# Check whether EUT can still send out data segment or not.
#
UpdateTcpSendBuffer TCB -c $ACK -w 0
SendTcpPacket TCB

#
# Call Tcp4.Transmit to transmit an urgent packet.
#
BS->CloseEvent "@R_Transmit_CompletionToken.Event, &@R_Status"
GetAck
BS->CreateEvent "$EVT_NOTIFY_SIGNAL, $EFI_TPL_CALLBACK, 1, &@R_Context,        \
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4UrgHandlingFunc3AssertionGuid002                  \
                "Tcp4.Transmit - Transmit a packet."                           \
                "ReturnStatus - $R_Status, ExpectedStatus - $EFI_SUCCESS"

#
# Check whether the urgent data can be sent out or not.
#
ReceiveTcpPacket TCB 5
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 || ${TCB.r_f_urg} != 1} {
    set assert fail
    RecordAssertion $assert $GenericAssertionGuid                              \
                    "EUT doesn't send out the probe segment correctly."
    ...
    return
      ...
      if { ${TCB.r_len} > 0 } {
       	set assert fail
        RecordAssertion $assert $GenericAssertionGuid                          \
                        "The data length of the probe segment is incorrect."
  ...
  set assert pass
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "The EUT send out the data probe segment correctly."
} else {
  set assert fail
  RecordAssertion $assert $GenericAssertionGuid                                \
                  "EUT doesn't send out any segment."
