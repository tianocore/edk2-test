...
#
CaseGuid          321AC81F-EA90-27a8-8582-FA2BC9E6AC12
...
BeginLogPacket Receive.Func3.Case2 "host $DEF_EUT_IP_ADDR and host             \
                                             $DEF_ENTS_IP_ADDR"
#
...
# OS send DATA & ACK segment
#
set tmp_seq   [ expr ${TCB.l_isn} + ${TCB.l_next_seq} - $L_OverlapLength ]
set L_TcpFlag [ expr $PSH | $ACK ]
...
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Tcp4ReceiveFunc3AssertionGuid002                      \
...
if { ${TCB.received} == 1 } {
  if { ${TCB.r_f_ack} != 1 } {
    ...
    if { ${TCB.r_ack} != [ expr $L_FragmentLength_1 + $L_FragmentLength_2 - $L_OverlapLength ] + 1 } {
      puts "{TCB.r_ack} = ${TCB.r_ack}\n"
      puts "received data length should be [ expr $L_FragmentLength_1 + $L_FragmentLength_2 - $L_OverlapLength ]\n"
    }
  }
} else {
  set assert fail
