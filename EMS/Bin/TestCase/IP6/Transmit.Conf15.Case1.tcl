...
#
CaseGuid           9FB4485E-A5D7-4bad-8147-98C4F5C81690
CaseName           Transmit.Conf15.Case1
CaseCategory       IP6
CaseDescription    { Test the Transmit Function of IP6 - invoke Transmit()                       \
                     when Token->Packet.TxData->DataLength is beyond the maximum fragment length \
                     EFI_BAD_BUFFER_SIZE should be returned.
                   }
################################################################################
...
# Begin Scope ...
#
BeginScope       IP6_TRANSMIT_CONF15_CASE1_
...
SetIpv6Address   R_TxData.DestinationAddress     "2002::1"
SetVar           R_TxData.OverrideData           0
SetVar           R_TxData.ExtHdrsLength          0
SetVar           R_TxData.ExtHdrs                0
SetVar           R_TxData.NextHeader             0
SetVar           R_TxData.DataLength             [expr int(pow(2,13) * pow(2,3) + 1)]
SetVar           R_TxData.FragmentCount          1

EFI_IP6_FRAGMENT_DATA                            R_FragmentTable
SetVar           R_FragmentTable.FragmentLength  [expr int(pow(2,13) * pow(2,3) + 1)]
...
#
# Check point:Call Transmit Function when Token->Packet.TxData->DataLength is beyond the maximum fragment length.
#             EFI_BAD_BUFFER_SIZE should be returned. 
#    
Ip6->Transmit    "&@R_Token, &@R_Status"
GetAck
set  assert      [VerifyReturnStatus R_Status  $EFI_BAD_BUFFER_SIZE]
RecordAssertion  $assert       $Ip6TransmitConf15AssertionGuid001                           \
                 "Ip6->Transmit -Conf- Call Transmit  with Token->Packet.TxData->DataLength \   
                 is beyond the maximum fragment length "                                                                                                  \
...
# End scope
#
EndScope         IP6_TRANSMIT_CONF15_CASE1_
#
# End log
#
EndLog
