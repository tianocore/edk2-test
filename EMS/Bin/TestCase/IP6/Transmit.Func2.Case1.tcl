...
proc CleanUpEutEnvironment {} {
  ...
  EndScope _IP6_TRANSMIT_FUNCTION2_CASE1_
...
# Begin Scope ...
#
BeginScope   _IP6_TRANSMIT_FUNCTION2_CASE1_
...
#
EFI_IP6_TRANSMIT_DATA_P2                       R_TxData
...
SetVar R_TxData.DataLength                     2000
SetVar R_TxData.FragmentCount                  2

EFI_IP6_FRAGMENT_DATA                          R_FragmentTable
set Ip6PayloadLength                           1000
SetVar R_FragmentTable.FragmentLength          $Ip6PayloadLength
CHAR8                                          R_FragmentBuffer(1100)
SetVar R_FragmentBuffer                        "IP6TransmitFunc2Test_FragmentTest."
...
ReceiveCcbPacket CCB Ip6Packet 5
ReceiveCcbPacket CCB Ip6Packet2 10
GetAck
set assert [VerifyReturnStatus R_Status $EFI_SUCCESS]
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid001       \
...
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid002       \
...
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid003       \
...
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid005   \
                "Ip6.Transmit - Func- First fragment ipv6 header check."

# Verify the second packet
ValidatePacket Ip6Packet2 -t ipv6 "(IPv6_ver=6)AND(IPv6_tc=0)AND(IPv6_fl=0)AND\
                             (IPv6_nh=0x2c)AND(IPv6_hl=64)\
                             (IPv6_dst=2002:0:0:0:0:0:0:2)AND(IPv6_src=2002:0:0:0:0:0:0:4321)"
...
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid006   \
                "Ip6.Transmit - Fragment Second fragment ipv6 header check."

ParsePacket Ip6Packet -t IPv6 -IPv6_pl IPV6PL
ParsePacket Ip6Packet2 -t IPv6 -IPv6_pl IPV6PL2

# Length excludes the two fragmentation header, the length of each is 8
puts [expr {[expr {$IPV6PL + $IPV6PL2}] - 16}]
if { [expr {[expr {$IPV6PL + $IPV6PL2}] - 16}] == 2000} {
  set assert pass
} else {
  set assert fail
}
RecordAssertion $assert $Ip6TransmitFunc2AssertionGuid007   \
                "Ip6.Transmit - Fragment Total fragment length check."
